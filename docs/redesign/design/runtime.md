...
| `RT-EVIDENCE`   | Immutable artifact store  | Data store   | Holds bounded evidence and configuration artifacts, pre-ack `SCH-INTAKE-ACK` configuration-read attempt variants, plus terminal audit exports. Attempt variants are mechanism evidence only; the `LG-INTAKE` terminal acknowledgement remains the sole intake authority.                                                                                                                                                                                                      |
...
| ---------------- | ------------------------------- | ------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `PORT-CONSUMER`  | First-party consumer API facade | CLI, private MCP adapter, SDK                                                                                             | Versioned first-party preview, pre-Run approval, start, inspection, decision, and export methods. Preview invokes `EP-PREVIEW`; approval invokes `EP-APPROVE` only after binding the caller to Arye's configured `ID-PRINCIPAL` and exact proposal digest/scope. Submission/core commands delegate to `PORT-INTAKE`, `PORT-DECIDE`, or `PORT-PUBLISH` without bypassing them. Every caller binds to exactly one configured principal or fails closed.                                                                                                                                                                                                                                                                                                                                        |
| `PORT-INTAKE`    | Envelope intake                 | `X-ENVELOPE`                                                                                                              | Composition-digest-keyed conditional-create submission in; byte-equivalent durable `SCH-INTAKE-ACK` `terminal-ack`, existing `ID-RUN` on duplicate/lost-ack lookup, and preflight outcome out. Pre-ack attempt variants arrive only through `PORT-ARTIFACT` and grant no intake authority.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
| `PORT-ARTIFACT`  | Artifact persistence            | `X-STORE`                                                                                                                 | Immutable evidence and terminal audit-export writes, owner-authorized disposal, plus digest-verified reads. `CP-INTAKE` directs each pre-Run configuration-artifact attempt through `CP-MEDIATOR`, bound to exact subject, expected digest, composition digest, deterministic key/ordinal, `BND-WAIT-MECHANISM`, and `BND-RETRY`. The mechanism conditional-creates/replays immutable `SCH-INTAKE-ACK` `config-read-attempt-start` and `config-read-attempt-result` variants; a new start references the prior attempt and proves its result or elapsed deadline, so loss/crash cannot reset consumption. Exhaustion rejects intake before Run creation. The ordered handles enter the `terminal-ack`; attempt variants grant no intake authority and create no uncataloged Operation/event. |
...
- `PORT-CONSUMER` is a private product facade, not another lifecycle authority or provider seam. It
  ...
  the existing validated ports for core actions. Its preview and pre-Run approval methods instead
  invoke the Envelope Builder's `EP-PREVIEW` and `EP-APPROVE` paths outside the core authority
  boundary. Approval accepts only Arye's bound principal and records the exact proposal digest and
  scope; it uses no `PORT-DECIDE`, per-Run `ID-GRANT`, event, Transition, or dispatch. Controller
  components, stores, and mechanisms remain unreachable until the separately approved envelope is
...
| Approve exact pre-Run proposal                         | Envelope Builder `EP-APPROVE`                 | Requires Arye's authenticated `ID-PRINCIPAL`; records exact proposal digest and scope; no delegate, Run grant, event, or dispatch. |
