...
| `EP-PROVIDERS` | Bind each selected provider to an approved authority-manifest digest and qualifying conformance evidence; acquire the provider's `SCH-CAPABILITY-PROOF` through that provider's own configured mechanism-port exchange under per-attempt `BND-WAIT-MECHANISM` and total-attempt `BND-RETRY`; for the Agent provider, select one exact native permission posture and declared semantics. | A missing, changed, stale, insufficient, negative, timed-out, exhausted, or policy-incompatible posture/proof keeps the envelope unlaunchable and fails composition/preflight before Run creation.                                      |
...
capability proof is immutable recorded evidence referenced from the proposal. Every terminal
attempt, including timeout or failure, is a `SCH-CAPABILITY-PROOF` record under a deterministic
request key, with basis, ordinal, start/deadline, and bound consumption. Recovery queries that same
key before it can advance an ordinal, so loss or crash cannot reset `BND-WAIT-MECHANISM` or
`BND-RETRY`; exhaustion leaves no positive proof and fails composition/preflight before any Run,
never treating missing evidence as success.
...
attempt uses a deterministic request key, immutable `SCH-INTAKE-ACK` start/result variants,
`BND-WAIT-MECHANISM`, and one `BND-RETRY` ordinal. A lost response or process crash replays the same
start/result keys; a new ordinal proves the prior result or elapsed deadline, so consumption cannot
reset. Exhaustion rejects intake before Run creation, and the accepted/rejected `terminal-ack` binds
the complete attempt chain without creating an Operation or event. Applying one records its identifier and digest,
