...
| `SCH-CONFIG-ARTIFACT`    | Preset or prompt/role-artifact author; a generator produces only its declared generated content.                                         | Envelope Builder, work-profile resolution, and `CP-INTAKE`.             | `CP-INTAKE` performs the pre-Run `PORT-ARTIFACT` digest-verified read and records the exact subject and carrier digest in `SCH-INTAKE-ACK`; envelope validation rejects a missing or mismatched carrier. |
