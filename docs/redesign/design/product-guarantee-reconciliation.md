...
`upstream`, or `conflict` rows. Product-commitment result: 44 `satisfied` rows and zero explicit
...
| FENCE-3 | `SCH-PROVIDER-AUTHORITY` and `CB-SESSION` make forge and privileged-delivery credential classes structurally unrepresentable for Agent sessions; manifest/binding validation and adversarial `CF-MECH-SESSION` probes reject them as `FC-AUTHORITY`. Privileged effects remain in delivery bindings and recorded Transitions. | `satisfied`    |

### Earned trust (`EARN`)

| ID     | Carried by                                                                                                                                                                                                                                                                                                                                                                               | Classification |
| ------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------- |
| EARN-1 | `CF-GATE-PROVIDER` in [architecture conformance](./architecture-conformance.md): a reusable pass binds the exact provider build, suite and probe versions, owner-approved authority-manifest digest, and environment; every Run also requires fresh compose-time capability proof whose bounded immutable attempts use `LG-PREFLIGHT-ATTEMPT`, and policy may impose a maximum pass age. | `satisfied`    |
...
| MERGE-2 | Landing and target-changing Operations are proposed only by `CP-FINALIZER` under sole target authority and authorized only in a recorded `CP-TRANSITION` Transition. D15 review publication cannot merge, touch the lineage anchor, acquire authority, or land.                                                                             | `satisfied`    |
...
| MERGE-4 | Product `done` projects from `Finalizing` once the frozen final-verification requirement is satisfied — a required `deterministic` check passed, or the posture is `none` — delivery is authorized, and only landing remains; `landed` projects from `Landed`. Merge-queue and branch-protection holds are durable `BND-WAIT-TARGET` waits. | `note`         |
| MERGE-5 | D15 `OPC-REV-*` can create/update the draft review venue and surface `Blocked`; post-acceptance `OPC-DEL-*` maintains the landing venue. Terminal outcomes authorize the four `OPC-REV-RETIRE-*` cleanup classes, and surfacing/retirement failure preserves an `ID-OBLIGATION`.                                                            | `satisfied`    |
...
| SEC-3 | As FENCE-3: Agent manifests and `CB-SESSION` cannot represent forge or privileged-delivery credential classes, and `CF-MECH-SESSION` rejects adversarial attempts. Delivery mechanisms alone receive their narrowed binding; workers receive none.                                                    | `satisfied`    |
...
| ID     | Carried by                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | Classification |
| ------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------- |
| CFG-1  | Frozen policy supplies an explicit final-verification posture, explicit required check-class set, and exactly one explicit integration/merge-spectrum mode; omission of any of these selections fails preflight closed. It also supplies anti-gaming rule surfaces, provider-permission floors, capacity maxima/reserve, bounds, mechanism minima, and explicitly named non-gating classes; the design-owned acceptance, evidence-integrity, authority, landing, and preservation forbidden set is unrepresentable in an accepted envelope. It also supplies evidence size/behavior, retention, snapshot cadence, proof freshness, and notice delivery targets. Owner-reviewable defaults/ranges are explicit; repo floors cannot weaken them. | `satisfied`    |
...
| CFG-4  | `CP-SCHEDULER` derives admission from durable order, capacity, each Story's plan-declared path-to-safe-point demand (default one per used class), and the configured reserve (default one, range one through capacity minus one); `RC-FINALIZER` is the structural singleton exception.                                                                                                                                                                                                                                                                                                                                                                                                                                                        | `satisfied`    |
...
| RESUME-3 | One durable identity per semantic effect; same-identity retry only after confirmed absence under reauthorization, while effect-free replacements receive new identities; reconciliation precedes a second effect (I17), and readback plus `LG-WITNESS` expose duplicates/rollback.                      | `satisfied`    |
...
| ID     | Carried by                                                                                                                                                                                                                                                                             | Classification |
| ------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------- |
| LIVE-1 | `SCH-LIVENESS`, `EV-LIVENESS-OBSERVED`, `BND-IDLE`, and `BND-SILENCE` turn subject-bound facts into deterministic thinking/stuck/dead/input-overdue classifications; the fixed selector parks owner-changeable outcomes and directly blocks those with no authorized advancing action. | `satisfied`    |
| LIVE-2 | Each failure code plus durable subject facts selects one containment/outcome: owner-changeable waits park, terminal exhausted work blocks, and `FC-TRUST` stops; alerts derive from the same durable bound facts (I16).                                                                | `satisfied`    |
...
| STACK-2 | Work Source ↔ `PORT-SOURCE` with `ID-SOURCE-REQ`; Agent ↔ `PORT-SESSION`; the Execution Host seam is realized inside the `PORT-WORKSPACE` provider family through manifest-level host swap, setup-receipt host identity/fingerprint, native-posture enforcement, replacement semantics, and host-specific `CF-MECH-WORKSPACE` clauses; Forge ↔ disjoint `PORT-DELIVERY` bindings. | `satisfied`    |
...
| DRIVE-2 | `SCH-PROVIDER-AUTHORITY` is digest-bound and owner-approved through the distinct authenticated `PORT-CONSUMER` exact-manifest method, declaring supported native permission modes, runtime, filesystem, network, credential, and effect authority; any digest change requires a fresh pass and owner approval. | `satisfied`    |
...
| SEE-6 | Terminal `SCH-AUDIT-EXPORT` bytes are canonical, redacted, content-addressed, and cover the exact ledger range through the terminal-settlement position. The receipt/export-failure obligation are post-terminal records outside that range; recovery verifies or completes identical bytes, never overwrites them. | `satisfied`    |

Notice acknowledgement changes presentation only; the underlying durable condition must resolve.
The terminal export includes the terminal-settlement position and exact covered range. Its
immutable-store receipt is a post-terminal administrative record outside the export bytes, and a
digest mismatch remains an explicit integrity obligation.
...
| `PC-README-3` — a valid execution plan plus policy is Jig's only hard input                                                                                                             | `README.md` suite position                      | `SCH-PLAN`, its Envelope Builder validation rules, `SCH-ENVELOPE`, `PORT-INTAKE`, and `CP-INTAKE`                                                                                         | `satisfied`    |
...
| `PC-JIG-3` — preview is effect-free and start uses validated intake without equating submission with preflight success                                                                  | `jig.md` driving a run                          | `PORT-CONSUMER` → Envelope Builder `EP-PREVIEW` for preview; `PORT-CONSUMER` → `PORT-INTAKE`, `LG-INTAKE`, and `SCH-INTAKE-ACK` for start                                                 | `satisfied`    |
...
| `PC-JIG-5` — approve, reject, override, decide, and handoff are authoritative operator actions                                                                                          | `jig.md` driving a run                          | `PORT-CONSUMER` → `PORT-DECIDE`; `EV-OWNER-DECISION` for decisions; `EV-DELEGATION-GRANT`, `SCH-DELEGATION-GRANT`, and `ID-GRANT` for decision handoff                                    | `satisfied`    |
...
| `PC-JIG-7` — plans carry Stories, dependencies, and owner-policy done conditions                                                                                                        | `jig.md` execution plan                         | `SCH-PLAN`; Envelope Builder acyclicity, dangling-reference, done-condition/check-class, and exactly-one-track validation; approved-plan digest frozen in `SCH-ENVELOPE`; `RP-CHECKCLASS` | `satisfied`    |
| `PC-JIG-8` — Runner, Worker, Reviewer, and four mechanism roles remain authority-separated                                                                                              | `jig.md` run roles                              | V1/V2, I2/I3, `CB-*`, per-port `CF-MECH-*` suites, and the Execution Host contract inside the `PORT-WORKSPACE` provider family                                                            | `satisfied`    |
| `PC-JIG-9` — provider human requests follow the durable request and principal across session loss                                                                                       | `jig.md` run roles                              | D14, `ID-SESSION`, `ID-PARK`, `OPC-SESSION-RESPOND`, `MC-RECONNECT`, and `CF-MECH-SESSION`                                                                                                | `satisfied`    |
| `PC-JIG-10` — acceptance is reviewer-principal; deterministic/none is the additional final-verification posture                                                                         | `jig.md` acceptance                             | D7, `RP-INDEPENDENCE`, `ID-FINDING`, `SCH-VERDICT`, `CF-ACCEPTANCE`, and `OPC-VERIFY-EXECUTE`                                                                                             | `satisfied`    |
| `PC-JIG-11` — review may need a pushed branch or draft request before acceptance, without landing authority                                                                             | `jig.md` acceptance                             | D15, `CB-REVIEW-PUBLICATION`, create/update and terminal `OPC-REV-*` retirement Operations, and `CF-REVIEW-PUBLICATION`                                                                   | `satisfied`    |
...
| `PC-JIG-18` — v1 is local-first and ships no remote Execution Host expectation                                                                                                          | `jig.md` not-yet boundary                       | D2 plus the manifest-swappable Execution Host contract inside the local-first `PORT-WORKSPACE` provider family; no remote provider is assumed or required                                 | `satisfied`    |
| `PC-JIG-19` — runs are operator-initiated; webhook and scheduler triggers are deferred                                                                                                  | `jig.md` not-yet boundary                       | `PORT-CONSUMER` → `PORT-INTAKE` is the only first-party start path; preview is effect-free Builder evaluation; `CF-CONSUMER` rejects undeclared control paths                             | `satisfied`    |
...
| `PC-JIG-23` — the owner/operator job is bounded execution from approved inputs, replacing fragile manual session, PR, and recovery chains                                               | `jig.md` product spine/job                      | Project brief O2/O5/O8, D2 authority-and-proof boundary, and V3/V4 durable lifecycle/recovery                                                                                             | `satisfied`    |
| `PC-JIG-24` — Jig is not product definition, design authoring, general LLM project management, or a review bypass                                                                       | `jig.md` product spine/non-fit                  | D2/D13 boundaries, D7 reviewer-principal acceptance, and the private consumer/provider surfaces                                                                                           | `satisfied`    |
| `PC-JIG-25` — success is evidenced land-or-stop with ordinary recovery and lower unsafe surprise/review burden; product remains mechanism-free and defaults do not become product truth | `jig.md` success and counter-signals            | Brief O2/O5/O8, D1/I1/I21, owner-reviewable design defaults, this reconciliation, and exact-candidate gate/conformance records                                                            | `satisfied`    |
...
| `PC-CONCEPTS-9` — one independent Track scopes optional upstream artifacts and exactly one plan/policy/profile composition without false cross-track dependencies                       | `concepts.md` Track identity and chain          | D2/D13, `EP-TRACK`, `SCH-PLAN`, `SCH-ENVELOPE`, and independent per-Run controller/scheduling boundaries                                                                                  | `satisfied`    |
...
| `PC-USE-2` — risky provider questions park durably and answer without widening posture                                                                                                  | `use-cases.md` provider human-request paragraph | Normalizes to `PC-JIG-9`; D14, `ID-SESSION`, `ID-PARK`, `SCH-DECISION`, and request-follows-principal session contract                                                                    | `satisfied`    |
...
### Round-6 minimal-route audit

All 44 routes below were audited clause by clause against their corresponding commitment. A route
contains the smallest named element set that jointly covers the commitment's independent clauses;
an element is omitted only when another listed element already proves that exact clause. This table
is the normative `PC-*` input inventory for `CF-GATE-PRODUCT`; it intentionally does not create
individual routes for the 56 imported guarantee IDs, whose matrix-plus-suite disposition remains
the settled rounds-3–5 resolution.

| Commitment      | Minimal proof-route set                                                                                                                                                                                             |
| --------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `PC-README-1`   | Governance: D1 source scope + Layer 1/2 gate records hold                                                                                                                                                           |
| `PC-README-2`   | Governance: D2 boundary + D13 Envelope Builder boundary hold                                                                                                                                                        |
| `PC-README-3`   | `CF-ENVELOPE` pass + Governance: D2/D13 boundary records hold                                                                                                                                                       |
| `PC-JIG-1`      | `CF-ENVELOPE` pass + `CF-ACCEPTANCE` pass + `CF-POLICY` pass + `CF-RUN-CONTROL` pass + `CF-RELEASE` pass + `CF-NOTICE-EXPORT` pass + `CF-OBSERVABILITY` pass                                                        |
| `PC-JIG-2`      | `CF-ENVELOPE` pass + `CF-RULE-SURFACE` pass + `CF-FENCE` pass                                                                                                                                                       |
| `PC-JIG-3`      | `CF-CONSUMER` pass + `CF-ENVELOPE` pass                                                                                                                                                                             |
| `PC-JIG-4`      | `CF-CONSUMER` pass + `CF-OBSERVABILITY` pass                                                                                                                                                                        |
| `PC-JIG-5`      | `CF-OPERATOR-ACTIONS` pass + `CF-DELEGATION` pass                                                                                                                                                                   |
| `PC-JIG-6`      | `CF-OPERATOR-ACTIONS` pass + `CF-RUN-CONTROL` pass                                                                                                                                                                  |
| `PC-JIG-7`      | `CF-ENVELOPE` pass + `CF-POLICY` pass                                                                                                                                                                               |
| `PC-JIG-8`      | `CF-BINDING` pass + `EV-SESSION-VERDICT` independence validation holds + `CF-PROVIDER-AUTHORITY` pass + `CF-SECRET-ABSENCE` pass + `CF-MECH-SESSION`/`CF-MECH-WORKSPACE`/`CF-MECH-VERIFY`/`CF-MECH-DELIVERY` passes |
| `PC-JIG-9`      | `CF-MECH-SESSION` pass + `CF-PROVIDER-PERMISSION` pass                                                                                                                                                              |
...
| `PC-JIG-13`     | Governance: D2/D13 boundary records + V1/runtime-unit and port-ownership records hold                                                                                                                               |
| `PC-JIG-14`     | `CF-CONSUMER` pass + Governance: D2/D13 boundary records + runtime/D10 private-facade and no-public-stability records hold                                                                                          |
| `PC-JIG-15`     | `CF-GATE-PROVIDER` pass + `CF-PROVIDER-AUTHORITY` pass + Governance: D10/D12 mechanism records hold                                                                                                                 |
| `PC-JIG-16`     | `CF-GATE-PROVIDER` pass + Governance: D10 package deferral holds                                                                                                                                                    |
| `PC-JIG-17`     | Governance: D10 private-facade + D14 middleman-exclusion records hold                                                                                                                                               |
| `PC-JIG-18`     | `CF-MECH-WORKSPACE` pass + `CF-PROVIDER-AUTHORITY` pass + Governance: gate-record `B2` resolution and D14 exclusions hold                                                                                           |
| `PC-JIG-19`     | `CF-CONSUMER` pass + `CF-ENVELOPE` pass + Governance: D10/D13 boundary records hold                                                                                                                                 |
| `PC-JIG-20`     | Governance: gate-record `B2` resolution + D10 runtime-boundary records hold                                                                                                                                         |
| `PC-JIG-21`     | `CF-GATE-PROVIDER` pass + Governance: gate-record unproven-edge inventory holds                                                                                                                                     |
| `PC-JIG-22`     | `CF-ENVELOPE` pass, including the actionable legacy-guidance clause                                                                                                                                                 |
| `PC-JIG-23`     | `CF-ENVELOPE` pass + `CF-RUN-CONTROL` pass + `CF-BOUNDS` pass + `CF-OBSERVABILITY` pass                                                                                                                             |
| `PC-JIG-24`     | Governance: D2/D13 boundary + D7 acceptance records hold                                                                                                                                                            |
| `PC-JIG-25`     | `CF-CONTAINMENT` pass + `CF-RUN-CONTROL` pass + `CF-RELEASE` pass + `CF-OBSERVABILITY` pass + Governance: D1/D2 records hold                                                                                        |
| `PC-CONCEPTS-1` | `CF-ENVELOPE` pass + `CF-POLICY` pass                                                                                                                                                                               |
| `PC-CONCEPTS-2` | `CF-BLOCKERS` pass + `CF-CAPACITY` pass + `CF-RELEASE` pass                                                                                                                                                         |
| `PC-CONCEPTS-3` | `CF-BINDING` pass + `EV-SESSION-VERDICT` independence validation holds + `CF-PROVIDER-AUTHORITY` pass + `CF-SECRET-ABSENCE` pass + `CF-MECH-SESSION`/`CF-MECH-WORKSPACE`/`CF-MECH-VERIFY`/`CF-MECH-DELIVERY` passes |
| `PC-CONCEPTS-4` | `CF-CONSUMER` pass + Governance: D2/D13 boundary records + runtime/D10 private-facade and no-public-stability records hold                                                                                          |
| `PC-CONCEPTS-5` | `CF-GATE-PROVIDER` pass + `CF-PROVIDER-AUTHORITY` pass                                                                                                                                                              |
| `PC-CONCEPTS-6` | `CF-REVIEW-PUBLICATION` pass                                                                                                                                                                                        |
| `PC-CONCEPTS-7` | `CF-RUN-CONTROL` pass + `CF-OBSERVABILITY` pass + Governance: canonical product projection holds                                                                                                                    |
| `PC-CONCEPTS-8` | `CF-RUN-CONTROL` pass + `CF-OBSERVABILITY` pass + Governance: canonical product projection holds                                                                                                                    |
| `PC-CONCEPTS-9` | `CF-ENVELOPE` pass + Governance: D2/D13, `EP-TRACK`, and independent per-Run controller/scheduling boundaries hold                                                                                                  |
| `PC-USE-1`      | `CF-BOUNDS` pass + `CF-CAPACITY` pass + `CF-MECH-WORKSPACE` pass + `CF-OBSERVABILITY` pass                                                                                                                          |
| `PC-USE-2`      | `CF-PROVIDER-PERMISSION` pass + `CF-MECH-SESSION` pass                                                                                                                                                              |
| `PC-USE-3`      | `CF-POLICY` pass + `CF-PROVIDER-AUTHORITY` pass                                                                                                                                                                     |
| `PC-USE-4`      | `CF-FENCE` pass + `CF-RUN-CONTROL` pass                                                                                                                                                                             |
| `PC-USE-5`      | `CF-GATE-PROVIDER` pass + `CF-PROVIDER-AUTHORITY` pass                                                                                                                                                              |
| `PC-USE-6`      | `CF-OBSERVABILITY` pass + `CF-NOTICE-EXPORT` pass + `CF-EVIDENCE-LIFECYCLE` pass                                                                                                                                    |
