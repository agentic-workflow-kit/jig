...
| `SCH-CONFIG-ARTIFACT`    | Preset or prompt/role-artifact author; a generator produces only its declared generated content.                                         | Envelope Builder, work-profile resolution, and `CP-INTAKE`.             | `PORT-ARTIFACT` digest verification; envelope validation rejects a missing or mismatched carrier.   |
