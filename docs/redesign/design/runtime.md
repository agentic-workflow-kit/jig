...
| --------------- | ------------------------- | ------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `RT-OPERATOR`   | Operator interface        | Runtime unit | Implements the private first-party `PORT-CONSUMER` facade and hosts pre-controller `CP-INTAKE`. Intake's only durable authority write is the `LG-INTAKE` acknowledgement conditional-create; its earlier attempt records use the non-authoritative `LG-PREFLIGHT-ATTEMPT` commit primitive. After an **accepted** acknowledgement exists, the operator idempotently creates the derived Run ledger and spawns the controller. A rejected acknowledgement permits neither action. The unit otherwise delegates commands and presents durable outcomes; it holds no lifecycle authority.                                                                                                                                                                                                                                                                                                                                              |
...
| `PORT-INTAKE`    | Envelope intake                 | `X-ENVELOPE`                                                                                                              | Composition-digest-keyed conditional-create submission in; byte-equivalent durable `SCH-INTAKE-ACK` `terminal-ack`, the accepted result's existing `ID-RUN` or the rejected result's explicit absence on duplicate/lost-ack lookup, and preflight outcome out. Pre-ack attempt variants arrive only through `PORT-ARTIFACT` and grant no intake authority.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
| `PORT-LEDGER`    | Ledger commit and read          | `X-STORE`                                                                                                                 | Conditional ordered appends and verified reads of Run/registry authority, the deployment-scoped digest-to-intake-acknowledgement index `LG-INTAKE`, and the only advance/read path for witness heads, including the one deployment-wide artifact pin-lookup head.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| `PORT-ARTIFACT`  | Artifact persistence            | `X-STORE`                                                                                                                 | Immutable evidence and terminal audit-export writes, owner-authorized disposal only in the disposable evidence context, digest-verified reads, and that context's authoritative reverse-reference pin lookup. Holder class selects the context under the approved artifact-provider manifest/resource scope. The protected configuration/intake context rejects disposal, move, or cross-context aliasing; a changed, missing, or unverifiable binding fails closed. For a disposable pin add/release, the port durably flushes and returns the exact lookup `(position, head digest)`; `CP-EVIDENCE` supplies it to `CP-TRANSITION`, whose commit protocol advances the matching independent `LG-WITNESS` line through `PORT-LEDGER`, and only then may the enclosing action acknowledge its holder. Restart/restore verifies the exact witnessed lookup head before disposable adoption, release, or disposal. For configuration reads, it is the storage-facing path of `LG-PREFLIGHT-ATTEMPT`: `CP-INTAKE` directs each attempt through `CP-MEDIATOR`, bound by its holder class to the protected context, exact subject, expected digest, composition digest, one exchange-attempt key with derived start/result variant keys, ordinal, `BND-WAIT-MECHANISM`, and `BND-RETRY`. The commit primitive conditional-creates or reads back byte-equivalent immutable `SCH-INTAKE-ACK` variants at those separate keys. A new start must bind the prior terminal result or prove its deadline elapsed; variant mismatch, predecessor, deadline, digest, or integrity failure fails closed, so loss/crash cannot reset consumption. Exhaustion rejects intake before Run creation. The ordered handles enter the `terminal-ack`; attempt variants grant no intake authority and create no Operation or event. |
...
bypassing either one. `PORT-ARTIFACT` returns proof that the authoritative pin mutation durably
flushed; `CP-EVIDENCE` supplies that exact `(position, head digest)` to `CP-TRANSITION`, whose
existing witness-line `PORT-LEDGER` commit protocol withholds completion until the independent
`RT-WITNESS` advance is durable. Only then may the existing enclosing-action owner acknowledge or
adopt the holder. `PORT-ARTIFACT` never writes `RT-WITNESS` directly, and witness completion grants
...
| Ledger authority, record before adopt/dispatch (I5) | `RT-LEDGER` behind `PORT-LEDGER`; the transition engine alone writes Run Transitions. `CP-INTAKE` writes only `LG-INTAKE` as intake authority; the non-authoritative `LG-PREFLIGHT-ATTEMPT` and registry arbitration use their separately declared commit-primitive contracts.                                                                                                          |
