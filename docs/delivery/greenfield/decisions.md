...
is sufficient implementation authorization. It may select only the bounded defaults and `DR-*`
choices that the request actually names or necessarily incorporates. No separate package
qualification, digest approval, external approval issue, or landed-commit equivalence record is
required.
