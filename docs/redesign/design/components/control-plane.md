...
creates or looks up `LG-INTAKE`; that `SCH-INTAKE-ACK` `terminal-ack` conditional-create is its only
durable Jig-authority write and the single intake commit point. Before that conditional create,
...
point and the primitive is not a second ledger authority. Only after an
...
| Record              | `CP-TRANSITION`            | `FLOW-RECORD` is the only Run Transition-ledger conditional append path (I5). `LG-INTAKE` conditional-create and target-registry arbitration are separate named authority structures with their declared writers; `LG-PREFLIGHT-ATTEMPT` is a separate non-authoritative commit primitive for bounded pre-Run evidence.                                                                                                                                                                                                                     |
