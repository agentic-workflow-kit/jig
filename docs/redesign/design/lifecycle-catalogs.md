...
| `EV-DELEGATION-GRANT`           | Delegation changes        | `P-OWNER` via `PORT-DECIDE`           | Run and decision scope       | Issuer, delegate, exact per-Run event/decision and subject scope, validity window, and revocation or supersession lineage satisfy `SCH-DELEGATION-GRANT`; issuance proves the issuer may delegate that exact scope, and implicit subdelegation is rejected.           |
