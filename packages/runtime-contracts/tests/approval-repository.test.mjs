...
test('pre-Run approval repository is immutable, exact, and recoverable across instances', () => {
  ...
  const reopened = runtime.createLocalPreRunApprovalRepository(root);
...
test('pre-Run approval repository rejects conflicting replay and integrity loss without replacement', () => {
  ...
  const corrupt = runtime.createLocalPreRunApprovalRepository(corruptRoot);
...
test('approval records reject wrong principal, kind substitution, and malformed scope before persistence', () => {
  const { repository, record } = fixture();
