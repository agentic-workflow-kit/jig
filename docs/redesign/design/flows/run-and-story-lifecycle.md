...
| **Stopped**                  | A recorded `FC-TRUST` trust-root failure, including a liveness-assumption breach that destroys that root, or an explicit terminal-stop decision ended the Run. | Terminal durable non-delivery outcome when a trustworthy witnessed append basis or external recovery establishes it; projects as product `ended`, never resumable `stopped`.                                                |
