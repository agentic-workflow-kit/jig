...
For a live `open` obligation that blocks terminal settlement, the first existing
`EV-BOUND-EXHAUSTED` is the named no-settlement disposition. It leaves the obligation open and
owner-only handoff unchanged and preserves the current `Retiring`, `Settling`, or pre-terminal
`Stopped` position; it cannot auto-handoff, resolve, close, settle, authorize a terminal-settlement
audit export, or dispose. Exact owner acceptance or evidence-backed resolution later wakes
settlement and permits that terminal audit export only after the terminal position commits.
...
| **Completed**                | Every Story has a final business outcome and every Retirement duty is complete or represented by an exact Residual Obligation with status `accepted-handoff`.                                                                                  | Terminal durable Run result; later obligation overdue re-escalation, handoff acceptance, or resolution is post-terminal administration and does not revise these facts.                                                                                                                                                |
...
| `Preflighting` → `Active`                                                                             | `EV-ENVELOPE-SUBMITTED`                                                                                                                                                                                                                                                                                               | Accepted acknowledgement exists; for a successor its witnessed same-position cut-claim pair exists; derived Run ledger exists; frozen digest matches                                                                                              | First `SCH-EVENT`/`SCH-TRANSITION`, frozen Run basis, and current `ID-GEN`. A rejected same-cut contender creates no ledger or controller.                                                                                                                                                                                                                                                                                                                                    |
...
| `Interrupted / Recovering` → `Stopped`                                                                | `EV-RECOVERY-OBSERVATION`                                                                                                                                                                                                                                                                                             | `FC-TRUST` is selected and a trustworthy witnessed append basis remains for the final fenced record                                                                                                                                               | Named trust-root stop, externally governed recovery requirement, and one `SCH-SETTLEMENT opened` carrier under `ID-SETTLEMENT` with the preserved Story snapshot, finite safe duties, and retained fences. If no trustworthy append basis remains, phase one fences and halts without claiming an in-ledger stop or overlay; externally governed recovery owns the terminal disposition.                                                                                      |
...
| `Suspended` → `Stopped`                                                                               | `EV-RUN-TERMINAL-STOP-DECISION`                                                                                                                                                                                                                                                                                       | Explicit terminal intent and no resumable transition remains                                                                                                                                                                                      | Terminal non-delivery outcome and one `SCH-SETTLEMENT opened` carrier under `ID-SETTLEMENT`, binding every non-terminal Story's preserved underlying state, dispatch fence, complete deterministic duty inventory, and every retained finalization fence; no Story state or business outcome changes.                                                                                                                                                                         |
| Terminal `Stopped` before its settlement cut → same phase                                             | `EV-SESSION-FACT`, `EV-WORKSPACE-PRESERVED`, `EV-WORKSPACE-FACT`, review-retirement `EV-TARGET-FACT`, `EV-ARTIFACT-FACT`, `EV-EFFECT-CERTAINTY`, `EV-RECOVERY-OBSERVATION`, or `EV-WAKE-SETTLEMENT`                                                                                                                   | The fact binds one exact open Settlement duty or retained fence; the terminal-settlement cut has not committed                                                                                                                                    | Append `SCH-SETTLEMENT progress`, update only that duty/fence and the remaining-duty digest, create an exact `ID-OBLIGATION` only on bounded automatic-duty failure, and authorize only the next cataloged settlement or reconciliation intent. Story state, outcome, dependencies, and business authority remain frozen.                                                                                                                                                     |
...
| `Active`, `Parked`, `Suspended`, `Settling`, or terminal `Stopped` before settlement cut → same phase | `EV-BOUND-EXHAUSTED`                                                                                                                                                                                                                                                                                                  | An exact live `open` Residual Obligation reached its original `BND-WAIT-DECISION` deadline and no prior exhaustion fact exists                                                                                                                    | If it blocks terminal settlement, this first exhaustion is the named no-settlement disposition: it ends the bounded wait while leaving obligation status, owner-only handoff, and current positions unchanged; it cannot hand off, resolve, close, settle, authorize a terminal-settlement audit export, or dispose. Later exact owner acceptance or evidence-backed resolution exits it and wakes settlement. If already terminal, the re-escalation is administrative only. |
| `Settling` or terminal `Stopped` before settlement cut → same phase                                   | `EV-OWNER-DECISION`                                                                                                                                                                                                                                                                                                   | Arye accepts the exact live `open` `ID-OBLIGATION` and its completion criteria                                                                                                                                                                    | Status becomes `accepted-handoff`; exact replay appends nothing, and the next settlement wake re-evaluates the remaining duty set.                                                                                                                                                                                                                                                                                                                                            |
| `Settling` or terminal `Stopped` before settlement cut → same phase                                   | `EV-OBLIGATION-RESOLVED`                                                                                                                                                                                                                                                                                              | The exact live `open` or `accepted-handoff` duty completed; responder/current grant, completion criteria, and digest-verified evidence pass                                                                                                       | Status becomes terminal `resolved`; exact replay appends nothing, and the next settlement wake re-evaluates the remaining duty set.                                                                                                                                                                                                                                                                                                                                           |
...
overdue-obligation re-escalation, obligation handoff acceptance or resolution, wake-driven holder
retirement and its post-commit `release-pin` intent/result, and owner-authorized `dispose-bytes`.
They cannot alter any Run
...
| Residual Obligation overdue re-escalation    | `EV-BOUND-EXHAUSTED`     | Exact live status is `open`, the original `BND-WAIT-DECISION` deadline expired, and no prior exhaustion fact exists; append one overdue/re-escalation and notice basis without resetting the wait or changing obligation status, owner-only handoff authority, or settled facts.   |
...
| Evidence holder retirement/release intent    | `EV-WAKE-SETTLEMENT`     | The exact post-terminal holder liveness/retention basis ended; commit that holder's retirement and one mode-bound `release-pin` intent. Dispatch only after this Transition commits; it cannot delete bytes or alter settled facts.                                                |
| Evidence byte-disposal authorization         | `EV-OWNER-DECISION`      | Arye alone authorizes `dispose-bytes` for one exact disposable-context digest after holder-class routing and provider-resource binding, preservation, retention, obligation, audit-hold, live-pin, and current witnessed-head guards pass; append only the Operation intent.       |
| Evidence pin-release or byte-disposal result | `EV-ARTIFACT-FACT`       | For `release-pin`, the exact holder-retirement Transition already authorized the Operation; release cannot delete and uncertainty preserves the pin. For `dispose-bytes`, the exact owner decision and destructive guards pass. Append only the mode-bound result and lookup head. |
...
The terminal-stop Transition keeps every non-terminal Story in its existing Story state and creates
one durable `SCH-SETTLEMENT opened` carrier. That Run-scoped Settlement overlay, not a Story-state
transition, freezes business dispatch and drives preservation, session close, review-venue and
workspace retirement, artifact-holder duties, authority release after reconciliation, and
obligation handoff. Every result commits one causal `progress` variant and derives the next
`EV-WAKE-SETTLEMENT` regardless of completion order. The final wake atomically seals the carrier
only after every duty is complete or has exact `accepted-handoff`; that position authorizes the
unchanged post-terminal administrative regime, including `SCH-AUDIT-EXPORT`/`ID-EXPORT`.
...
| `Completed`                 | `Settling` at the terminal-settlement position                      | Only settlement-authorized audit export and receipt/failure, obligation administration, wake-driven holder retirement and its `release-pin` intent/result, and owner-authorized `dispose-bytes`. None may change phase, Story state, business-final cut, outcome, authority, or dependency. |
| `Stopped`                   | Trust-root stop in Recovery or terminal-stop decision               | `Stopped` settlement-duty self-appends until the terminal-settlement position; thereafter the same post-terminal administrative regime as `Completed` applies. It never resumes.                                                                                                            |

## Story lifecycle

| Story stage                  | Meaning and authority                                                                                                                                                                                                                                |
| ---------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Pending**                  | Derived structural ineligibility, not an [R5.1](../../guidelines/readiness-closure-rubric.md) wait: hold for every prerequisite to be confirmed `Landed`, with no `BND-*` class, owner-held bound, or exhaustion timer; allocate no Story resources. |
...
When an exact disposable holder's recorded liveness or retention basis ends before terminal
settlement, either the cataloged Story `Retiring` self-loop or terminal `Stopped` Settlement
overlay consumes `EV-WAKE-SETTLEMENT`: it commits holder retirement and a post-commit `release-pin`
intent without changing Story state. After terminal settlement, the post-terminal administrative
holder-retirement row owns the same wake. No other phase consumes it.
