...
durable Jig-authority write and the single intake commit point. Before that conditional create, `CP-INTAKE` sends each
referenced `SCH-CONFIG-ARTIFACT` lookup through `CP-MEDIATOR`'s intake-scoped `PORT-ARTIFACT`
exchange, bound to the exact artifact subject, expected digest, and envelope composition digest.
Each attempt is bounded by `BND-WAIT-MECHANISM`; a retry consumes `BND-RETRY`, and exhaustion rejects
the acknowledgement before any Run exists. Through `PORT-ARTIFACT`, the mechanism conditionally
creates/replays an immutable `SCH-INTAKE-ACK` `config-read-attempt-start` variant before the read and
an immutable `config-read-attempt-result` variant for a validated result/failure. After a crash or
lost response, `CP-INTAKE` re-queries the same deterministic key and preserves its deadline and
consumption; the next ordinal's start must reference a terminal result or prove the prior deadline
elapsed. The accepted/rejected `terminal-ack` binds the complete ordered attempt chain.
`CP-MEDIATOR` returns only validated mechanism evidence; this
pre-Run exchange creates no `SCH-OPERATION`, `SCH-EVENT`, or `EV-ARTIFACT-FACT`. Only after an
...
- `CP-MEDIATOR` is the sole gatekeeper for inbound messages on the mediated mechanism ports,
  ...
  key; the mechanism's immutable `SCH-INTAKE-ACK` start/result variants can only support bounded
  recovery and the terminal acknowledgement conditional-create and never create a trigger, fact, or
  Operation.
