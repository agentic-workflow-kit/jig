...
write and the single intake commit point. Before that conditional create, `CP-INTAKE` sends each
...
`CP-MEDIATOR` returns only a validation result; this pre-Run exchange creates no `SCH-OPERATION`,
`SCH-EVENT`, or `EV-ARTIFACT-FACT`. Only after an **accepted** acknowledgement exists may the operator
...
- `CP-MEDIATOR` is the sole gatekeeper for inbound messages on the mediated mechanism ports,
  ...
  mediator with the exact subject, expected digest, and composition digest; the result can only
  support the `SCH-INTAKE-ACK` conditional-create and never creates a trigger, fact, or Operation.
