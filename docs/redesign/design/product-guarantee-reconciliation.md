...
| MERGE-4 | Product `done` projects only from `Finalizing` after policy-required final verification passes and before confirmed landing; `landed` projects from `Landed`. A held integration remains a durable wait and only landing releases dependencies.                  | `note`         |
...
| `done`                  | Story `Finalizing` plus a passed policy-required final-verification result              | Final verification has passed and only landing remains; `Accepted` alone is not `done`.                                                                    |
...
| `stopped` (resumable)   | Run `Suspended`                                                                         | A validated operator suspend decision durably fences dispatch; Story states do not change and resume requires a new generation plus `RC-RESUME-INTEGRITY`. |
| `stopped` (terminal)    | Run `Stopped`                                                                           | Trust/liveness assumption failure or an explicit terminal-stop decision ended the Run; no resume transition exists.                                        |
...
| `PC-JIG-7` — plans carry Stories, dependencies, and owner-policy done conditions                                | `jig.md` execution plan                    | `SCH-PLAN`, Story graph validation, frozen policy, and `RP-CHECKCLASS`                                                                  | `satisfied`    |
...
| `PC-CONCEPTS-8` — product stopped is resumable suspension while design terminal `Stopped` never resumes         | `concepts.md` outcomes                     | Run V3a, `EV-RUN-*` events, V4 resume integrity, and projection above                                                                   | `satisfied`    |
