...
The twelve classes above govern these fifteen semantic surfaces. This is the finite R5.1 inventory:
each row names the accountable owner, durable reason/start, wake/completion, bound/reset rule, and
exhaustion action. The last three rows reuse existing classes; they introduce no new bound, event,
...
| 12  | Retirement duty                        | Settlement through `CP-TRANSITION`; terminal duty and accountable handoff owner are recorded.                                                                                                 | `EV-WAKE-SETTLEMENT` from each duty-completing fact.                              | `BND-RETIRE`, no renewal; exhaustion creates an owner-accountable `ID-OBLIGATION`.                                                                                                                                        |
| 13  | Compose-time capability-proof exchange | `EP-PROVIDERS`; each immutable `SCH-CAPABILITY-PROOF` attempt records deterministic request key/basis, provider/build/manifest/environment, ordinal, start/deadline, result, and consumption. | Positive subject/freshness-valid proof; lost response/crash replays the same key. | `BND-WAIT-MECHANISM` plus `BND-RETRY`; same-key discovery cannot reset consumption, and exhaustion leaves no positive proof and fails proposal/intake before Run creation.                                                |
| 14  | Pre-Run configuration-artifact read    | `CP-INTAKE`, validated by `CP-MEDIATOR`; immutable `SCH-INTAKE-ACK` start/result variants record exact subject/digest, composition, deterministic key/ordinal, deadline, and predecessor.     | Digest-valid result; lost response/crash replays the same attempt keys.           | `BND-WAIT-MECHANISM` plus `BND-RETRY`; a new start proves prior result/deadline, so consumption cannot reset. The `terminal-ack` binds the chain; exhaustion rejects intake before Run creation. Non-Operation/non-event. |
...
`EV-WAKE-AUTHORITY`. Transient in-process timers only prompt re-evaluation and carry no decision authority (D8);
