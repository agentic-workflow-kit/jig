...
function initializeFixtureHistory(workspace) {
  for (const [args, options] of [
    [['init', '-q'], {}],
    [['config', 'user.name', 'GF-001 fixture'], {}],
    [['config', 'user.email', 'gf-001@example.invalid'], {}],
    [['add', '.'], {}],
...
function task(summary, packageName) {
  const expected = `@gf-001-fixture/${packageName}#build`;
...
function assertExecutedSuccess(summary, packageName, expectedCache) {
  ...
  assert.equal(
    entry.taskId,
    `@gf-001-fixture/${packageName}#build`,
    `${packageName} evidence must remain subject-bound`,
  );
}

function withWorkspace(runCase) {
  const workspace = mkdtempSync(join(tmpdir(), 'gf001-turbo-test-'));
  let failure;
  try {
  ...
  try {
    ...
    throw failure
      ? new AggregateError([failure, cleanupError], 'GF-001 fixture test and cleanup failed')
      : cleanupError;
  }
  if (failure) throw failure;
}

test('GF-001: cold isolated cache executes the canonical active task graph', { concurrency: false }, () => {
...
test('GF-001: unchanged isolated warm run hits only local cache', { concurrency: false }, () => {
  withWorkspace(({ workspace, env }) => {
...
test('GF-001: reproducible Git-history affected selection runs only the changed producer and its dependent', {
  concurrency: false,
}, () => {
  withWorkspace(({ workspace, env }) => {
...
test('GF-001: shared compiler configuration invalidates every warm task and cannot restore stale output', {
  concurrency: false,
}, () => {
  withWorkspace(({ workspace, env }) => {
    const cold = runTurboWithSummary(['run', 'build'], workspace, env);
...
test('GF-001: uncached failure isolates a successful unrelated lane while preserving aggregate failure', {
  concurrency: false,
}, () => {
  withWorkspace(({ workspace, env }) => {
    const producer = join(workspace, 'packages/pkg-a/src/index.ts');
...
test('GF-001: a package script change invalidates that package cache entry', { concurrency: false }, () => {
  withWorkspace(({ workspace, env }) => {
    const cold = runTurboWithSummary(['run', 'build'], workspace, env);
    assert.equal(cold.error, null, cold.output);
    const warm = runTurboWithSummary(['run', 'build'], workspace, env);
