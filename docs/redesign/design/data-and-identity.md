...
| `ID-PARK`      | Human interaction      | `<run>/park/<ordinal>`                                   | Monotonic per Run; binds the exact Jig question or Agent-provider human-needed request, its origin/session, authorized responder scope, and the later selected answer.                                                                                                                                  |
...
| `SCH-ESCALATION`         | Escalation and parked request            | Origin kind (`jig` or `agent-provider`), request kind (`permission` or `question` for provider requests), exact Run/Story/session/request binding, bounded prompt and choices or requested scope, authority scope, wake condition, and deadline class.                                                                                                |
| `SCH-DECISION`           | Human answer                             | Responder identity, validated delegated scope, exact parked-request binding, the scoped permission decision or question answer, and continuation or stop. For provider requests it grants no Jig Operation and cannot change the selected provider posture.                                                                                           |
