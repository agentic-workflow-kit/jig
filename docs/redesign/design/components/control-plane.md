...
| `CP-MEDIATOR`   | Mechanism mediator             | The trust boundary inside the unit: dispatches authorized operations to mechanism ports and validates every inbound port message before it can become a trigger.      | `R-VALIDATE` (V2); I7.                                                                                    | `PORT-SESSION`, `PORT-WORKSPACE`, `PORT-VERIFY`, `PORT-DELIVERY`; inbound gate for all ports. |
| `CP-EVIDENCE`   | Evidence binder                | Assembles evidence manifests, binds each artifact to its exact subject, persists immutable artifacts, and validates availability and integrity before decision use.   | `A-EVIDENCE` roles and the exact-subject binding of `A-CANDIDATE` (V4); I7.                               | `PORT-ARTIFACT` (immutable writes; digest-verified reads).                                    |
...
- `CP-MEDIATOR` is the sole gatekeeper for inbound port messages. A message that fails identity,
  role, exact-subject, lifecycle, fence, or capability validation creates no trigger and no fact.
...
flowchart LR
    ...
    Evidence -->|"persists and digest-verifies artifacts via"| PArtifact
