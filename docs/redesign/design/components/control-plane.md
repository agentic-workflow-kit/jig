...
| --------------- | ------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------ |
| `CP-INTAKE`     | Intake and preflight           | Validates the complete submitted envelope — policy floors, work profile, provider-authority manifests, rule surfaces, setup recipe, and approvals — assigns stable Run identity, runs deterministic preflight, and freezes the Run definition or rejects durably. | `RUN-RECEIVED`, `RUN-PREFLIGHT`, `RUN-REJECTED` (V3); the frozen-envelope binding of the canonical model. | `PORT-INTAKE` (inbound, behind `CP-MEDIATOR`).                                                                                                   |
| `CP-TRANSITION` | Transition engine              | Executes the single-transition cycle for every accepted trigger: validate against lifecycle position, classify assisted/manual authority and changed rule surfaces deterministically, decide, record, then adopt and dispatch.                                    | `FLOW-VALIDATE`, `FLOW-DECIDE`, `FLOW-RECORD`, `FLOW-ADOPT` (V3c); I4, I5.                                | `PORT-LEDGER` (sole writer; conditional ordered appends).                                                                                        |
...
- Assisted authority is computed, never inferred from a provider. Credentials, delivery effects,
  destructive or irreversible actions, rule-surface changes, and unknown classes always require a
  human decision; only policy-declared reversible, non-privileged actions may auto-grant.
