export class PlanValidator {
  static validate(planInstance) {
    ...
    if (/[/\\..]/.test(plan.id)) {
      throw new Error(
        `Invalid plan: unsafe "id" containing path traversal characters: "${plan.id}"`,
      );
    }
    ...
    for (const story of plan.stories) {
      if (!story.id || !story.title) {
        throw new Error(
          `Invalid story: missing "id" or "title" in ${JSON.stringify(story)}`,
        );
      }
      ...
      if (typeof story.id !== "string" || /[/\\..]/.test(story.id)) {
        throw new Error(
          `Invalid story: unsafe "id" containing path traversal characters: "${story.id}"`,
        );
      }
      ...
      if (story.dependsOn) {
        if (!Array.isArray(story.dependsOn)) {
          throw new Error(
            `Invalid story: "dependsOn" must be an array in "${story.id}"`,
          );
        }
        for (const depId of story.dependsOn) {
          if (depId === story.id) {
            throw new Error(
              `Invalid story: self-dependency not allowed in "${story.id}"`,
            );
          }
