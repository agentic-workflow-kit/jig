...
stateDiagram-v2
    ...
    Suspended --> Suspended: EV-RECOVERY-OBSERVATION or EV-EFFECT-CERTAINTY, last target-changing Operation reconciled; durable authority release
...
durable terminal preflight fact and no controller is spawned. A rejected submission creates no
Run, no ledger, and no export: its immutable rejected `SCH-INTAKE-ACK` is the sole durable record
of the attempt and is readable by its digest lookup. `SCH-AUDIT-EXPORT` is structurally
inapplicable because this path never has a terminal-settlement position.
...
| `Active` or `Parked` → `Suspended`                             | `EV-RUN-SUSPEND-DECISION`                                                                                                                                                                                                                                                                                             | Exact Run/lifecycle position and responder/current grant pass; every in-flight target-changing Operation of a holding Story is attested, confirmed, cancelled with proof of no effect, or parked as `Uncertain` under `BND-RECOVERY` before authority release | Dispatch fence, reason, and preserved Story states; while any target-changing Operation remains unreconciled, finalization authority is retained-but-fenced rather than released, and the Run still suspends.                                             |
| `Suspended` → `Suspended`                                      | `EV-RECOVERY-OBSERVATION` or `EV-EFFECT-CERTAINTY`                                                                                                                                                                                                                                                                    | The fact completes the last in-flight target-changing Operation of the holding Story as attested, confirmed, cancelled with proof of no effect, or parked `Uncertain` under `BND-RECOVERY`; the dispatch fence remains in force                               | Phase remains `Suspended`; the Transition records reconciliation provenance and durably releases the retained authority. This is legal reconciliation-duty processing only and dispatches no Operation.                                                   |
...
| `Stopped` → `Stopped`                                          | `EV-WAKE-SETTLEMENT`                                                                                                                                                                                                                                                                                                  | Every terminal-stop settlement duty is complete or represented by an accepted `ID-OBLIGATION`                                                                                                                                                                 | Terminal-settlement position for the stopped Run; the existing post-terminal administrative regime then applies unchanged.                                                                                                                                |
...
existing obligation resolution, and owner-authorized evidence disposal. They cannot alter any Run
...
which durably releases retained authority when the last such Operation has its required disposition.
A held authority remains retained-but-fenced, rather
than released, until every in-flight target-changing Operation of its holding Story is attested,
confirmed, cancelled with proof of no effect, or parked as `Uncertain` under `BND-RECOVERY`.
...
every in-flight target-changing Operation of the holding Story to be attested, confirmed,
cancelled with proof of no effect, or parked as `Uncertain` under `BND-RECOVERY`. Until that
condition is met, the named Story or Run transition still records its state change but retains the
authority fenced; the existing reconciliation-completing Transition releases it durably.
...
| ------------------------------ | ----------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `Landed`                       | Dependents become eligible immediately after authoritative proof. Cleanup cannot reverse the outcome. | Release finalization authority only after every in-flight target-changing Operation is attested, confirmed, cancelled with proof of no effect, or parked `Uncertain` under `BND-RECOVERY`; until then retain it fenced, settle operations, preserve evidence, close sessions, and retire or hand off resources.     |
...
| `Rejected`                     | Transitive dependents become ineligible immediately from the recorded owner decision.                 | Release any authority only after every in-flight target-changing Operation is attested, confirmed, cancelled with proof of no effect, or parked `Uncertain` under `BND-RECOVERY`; until then retain finalization authority fenced, preserve work/evidence, close sessions, and retire or hand off resources safely. |
