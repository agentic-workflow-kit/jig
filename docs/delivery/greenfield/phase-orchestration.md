...
The implementer commits the candidate and runs the story's required checks in its registered worktree. The ledger binds the exact candidate commit, base, required command/set and result, timestamp and durable log reference, clean status before/after review, reviewer identity, independence, findings and verdict. The reviewer reads the frozen `HEAD`, base, diff, predecessor containment, checks, and status under the [reviewer protocol](./reviewer/README.md); it never runs checks or mutates state.

A fix or target refresh produces a new commit. The same implementer reruns affected required checks and the same reviewer incrementally reviews the prior reviewed candidate through the new one, including affected invariants and sibling occurrences. A prior verdict never authorizes the new commit. Record the cumulative verdict against the new candidate.
