...
| `CB-STORE`              | `PORT-LEDGER`/`PORT-ARTIFACT` | For Run-scoped `PORT-ARTIFACT`: one named artifact write/read, minted per Operation and validated by `CP-MEDIATOR`. The pre-Run configuration-artifact exception binds exact artifact subject/digest, envelope composition digest, deterministic attempt key/ordinal, start/deadline/result, predecessor attempt, and `BND-WAIT-MECHANISM`/`BND-RETRY`; `X-STORE` conditional-creates/replays the immutable `SCH-INTAKE-ACK` start/result variants, `CP-MEDIATOR` validates them, and no Operation/event or intake authority exists. For `PORT-LEDGER`: one intake composition-digest key minted by `CP-INTAKE`; or one Run's positions, one realization-bound registry line, or corresponding witness heads minted by the transition-engine commit protocol.                                                         |
...
| Artifact (`PORT-ARTIFACT`)   | Run-scoped immutable put/get/dispose is idempotent or effect-free under exact Operation identity. Separately, the pre-Run configuration read binds exact subject, expected digest, composition digest, attempt, and bounds without creating an Operation/event.                                                                                      | Run results return through `EV-ARTIFACT-FACT`; the pre-Run read returns only validated intake evidence. Every attempt uses `BND-WAIT-MECHANISM`; retries consume `BND-RETRY`; effectful uncertainty uses `BND-RECOVERY`.                            | Reconnection never replays an effect blindly. Put/dispose reconcile first; a config-read exhaustion rejects intake before Run creation. Preservation, retention, obligation, and cross-Run reference guards remain mandatory. |
...
`EP-PROVIDERS`, the Envelope Builder produces immutable `SCH-CAPABILITY-PROOF` attempt records
through the provider's own configured mechanism-port exchange. Each exact request has a
deterministic key, basis, ordinal, start/deadline, result, and `BND-WAIT-MECHANISM`/`BND-RETRY`
consumption. Recovery queries the same key before advancing, so loss or crash cannot reset a wait;
exhaustion leaves no positive proof and fails proposal/intake before any Run. The builder records
only the successful exact-subject proof by envelope reference. Every Run
