...
- Candidate-sensitive work binds the Story, `ID-CAND`, and the carrier-recorded target-basis
  digest from `SCH-CANDIDATE`. Review publication binds its dedicated
  `CB-REVIEW-PUBLICATION` without finalization authority; target-changing work also binds the
...
independent witness. Only then does it reconstruct canonical state, enumerate pending and uncertain
Operations, reconcile external state, record observations, revalidate authorities, and resume. A
changed, missing, or unverifiable context binding, or any protected-context replacement/restore
without proof of exact append-only continuity, is `FC-TRUST`; protected addresses remain
non-disposable and cannot move or alias. Before disposable pin-currency verification, Jig adopts or
releases no disposable reference and authorizes no `OPC-ART-DISPOSE`. A lookup behind its witness,
forked, digest-mismatched, unverifiable, or missing acknowledged content is `FC-TRUST` and a
deliberate stop until exact-head externally governed reconciliation; absent independent witness
means autonomous restore cannot establish disposable pin currency.
...
flowchart LR
    ...
    subgraph Acceptance["Exact-Candidate acceptance"]
        Candidate["A-CANDIDATE<br/>SCH-CANDIDATE at ID-CAND: exact content and target basis<br/>[Subject]"]
