...
| `BND-WAIT-CAPACITY`  | Time a skipped eligible Story waits for capacity admission.                                           | 24h; range 1h–30d.                 | No automatic renewal; each admission recomputation re-derives the wait, while the bound covers continuous starvation. | Park the Story with escalation naming capacity starvation (`FC-CAPACITY`).                                                                |
...
wait. A `BND-WAIT-CAPACITY` wait is accountable to `CP-SCHEDULER` and wakes on
`EV-WAKE-CAPACITY`. Transient in-process timers only prompt re-evaluation and carry no decision authority (D8);
