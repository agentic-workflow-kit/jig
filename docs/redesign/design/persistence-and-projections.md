...
| `LG-INTAKE`            | Intake acknowledgement and cut-claim index | The deployment-scoped intake authority containing composition-keyed immutable `SCH-INTAKE-ACK` records and accepted-successor `SCH-INTAKE-CUT-CLAIM` records. Genesis acceptance and rejection create one acknowledgement entry. Accepted successor intake atomically creates the acknowledgement and the full predecessor-Run/quarantine-cut-keyed claim at one shared position; both commit or neither does. Same-digest replay reads the existing result; a different digest is distinct only when it does not name an occupied cut. The witnessed ordered head is the single intake commit point and authority of record.                                                                                                                                                          |
...
only Jig-authority write is one `LG-INTAKE` semantic commit: acknowledgement-only for genesis or
rejection, or the atomic acknowledgement/claim pair for an accepted successor.
...
ledger head. The `LG-INTAKE` acknowledgement or atomic acknowledgement/claim pair remains the
**single intake commit point**. Its
...
Run-ledger genesis/rebuild basis. For a successor it also contains the deterministic cut-claim key
and staged claim content digest. The claim digest covers schema/version, predecessor Run, full cut
position/digest, winning composition digest, and acknowledgement key while excluding itself and
every post-commit field. The acknowledgement domain excludes its own digest field, `ID-RUN`, the
...
acknowledgement. For an accepted successor, the conditional commit requires both the composition
key and cut-claim key to be absent and assigns both entries one intake position. The ordered head
commits the prior head plus that position and the canonical ordered pair of entry keys/content
digests. A same-cut different-digest contender witness-verifies the winner pair, creates only its
own durable rejected acknowledgement with reason `successor-cut-already-claimed` and the winner's
stable binding, and derives no `ID-RUN`.

Recovery treats a complete pair with a behind witness as one durable commit whose witness may be
advanced before adoption; witness-ahead, missing-companion, mismatched-position, wrong-winner,
forked, or unverifiable states are `FC-TRUST` and are never repaired one side at a time. A lost
response reads the same pair. `RC-RESUME-INTEGRITY` consumes only the witnessed winner pair;
rejected contenders never fence predecessor resume.
