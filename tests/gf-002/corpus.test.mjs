...
test('GF-002: runner-neutral golden corpus supplies exact inputs and oracles', () => {
  assert.equal(corpus.schemaVersion, 2);
  assert.equal(corpus.corpusVersion, 'gf-002-corpus.v2');
...
test('GF-002: deterministic property corpus replays canonical bytes from its declared seed', () => {
...
test('GF-002: independent process consumer derives the same corpus oracle', () => {
  const result = spawnSync(process.execPath, ['tests/gf-002/golden-consumer.mjs'], { encoding: 'utf8' });
...
test('GF-002: independent consumer rejects wrong immutable canonical, digest, and error oracles', () => {
  const root = mkdtempSync('/tmp/gf-002-corpus-');
  try {
    for (const [index, mutate] of [
      ...
      const result = spawnSync(process.execPath, ['tests/gf-002/golden-consumer.mjs', path], { encoding: 'utf8' });
