...
| `ID-OP`         | Operation identity     | `<txn>/op/<ordinal>`                                      | Ordinal within the first authorizing Transition. One semantic effect keeps this identity and payload basis across redispatch, while each redispatch records a fresh reauthorization and current fence.                                                                                                  |
...
| `ID-GRANT`      | Delegation grant       | `<run>/grant/<ordinal>`                                   | Monotonic per Run; binds issuer and delegate principals, exact event/decision and subject scope, validity window, and revocation/supersession lineage. The grant records scope but cannot exceed authority the issuer may delegate under the governing owner contract.                                  |
...
Redispatch preserves `ID-OP` and payload basis but is a fresh Jig authorization. Its authorizing
Transition records the retained identity/basis, current `ID-GEN`, and current `ID-AUTH` plus
`ID-REGISTRY` where finalization authority was reacquired, so audit identifies every dispatching
generation. Candidate, target basis, binding, and manifest remain exact and are revalidated.
...
| `SCH-TRANSITION`         | Transition record                        | Ordered validated trigger reference, deterministic decision, authorized Operation intents, expected prior position, and any redispatch reauthorization facts naming retained Operation/payload basis and refreshed fence generation.                                                                                                                                                               |
...
| `SCH-DELEGATION-GRANT`   | Per-Run delegation grant                 | `ID-GRANT`, issuer and delegate principals, exact allowed event/decision classes and subject selectors, valid-from/expiry, issue position, revocation/revoker/reason, and predecessor/supersession reference. Issuance validation proves the issuer may delegate that exact scope; no implicit subdelegation is representable.                                                                     |
| `SCH-DECISION`           | Human/operator decision                  | Exact Run, parked-request, Story, or notice subject; decision kind and reason; responder identity; for a non-owner, current `ID-GRANT`; exact request binding where applicable; scoped permission/question answer, continuation, reject-story, suspend, resume, terminal stop, acknowledgement, or snooze. Provider answers grant no Jig Operation and cannot change posture.                      |
...
| `SCH-NOTICE`             | Actionable attention notice              | Stable notice identity/version, durable source facts, subject, deterministic urgency, explanation, currently valid actions, wake/expiry condition, and acknowledgement identity/state or snooze wake condition derived only from the matching durable operator event.                                                                                                                              |
