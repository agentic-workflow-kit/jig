...
| `CP-EVIDENCE`   | Evidence binder                   | Assembles evidence manifests, binds each artifact to its exact subject, persists immutable artifacts, and validates availability and integrity before decision use. For any of the six controller-mediated disposable holder classes, it coordinates the existing authoritative reverse-reference pin guard: the pin mutation must durably flush, then its verified `(position, head digest)` enters `CP-TRANSITION`'s existing witness-line commit protocol before holder adoption or release is acknowledged. The five pre-Run classes use the protected non-disposable context and never enter this route.                                                                                                                                                                                 | `A-EVIDENCE` roles and the exact-subject binding of `A-CANDIDATE` (V4); I7.            | `PORT-ARTIFACT` through `CP-MEDIATOR` for holder-class context routing, the disposable pin mutation, and immutable artifact access; the artifact-reference-index witness line through `CP-TRANSITION`'s `PORT-LEDGER` commit protocol. |
...
- `CP-EVIDENCE` treats disposable pin currency as a safety guard on existing holder actions: it cannot
  acknowledge an authoritative reverse-reference pin add or release until the mutation has flushed
  ...
  advances `RT-WITNESS`; only after that verified success may the existing enclosing-action owner
  issue its acknowledgement. This path neither creates a new lifecycle/control authority nor
