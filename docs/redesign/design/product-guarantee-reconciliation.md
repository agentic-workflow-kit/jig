...
| FENCE-3 | Credentials resolve per configured mechanism and live only in controller and mechanism process memory; role sessions receive bounded inputs under `CB-SESSION`, never privileged credentials; privileged effects are proposed only by `CP-FINALIZER` under sole target authority and authorized in a recorded Transition. | `satisfied`    |
...
| SEC-3 | As FENCE-3 for forge credentials: the delivery mechanism holds them; `CP-TRANSITION` authorizes bounded D15 review publication, while `CP-FINALIZER` alone may propose landing effects under sole target authority for Transition authorization; role sessions never see credentials.                 | `satisfied`    |
...
| ID     | Carried by                                                                                                                                                                                                                                                                                       | Classification |
| ------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------------- |
| CFG-1  | Frozen policy supplies verification posture, required check classes, policy-selected integration/merge-spectrum mode, anti-gaming rule surfaces, provider-permission floors, capacity maxima/reserve, bounds, mechanism minima, non-gating classes, and retention; repo floors cannot weaken it. | `satisfied`    |
