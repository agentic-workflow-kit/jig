...
| INV-009  | Invariant     | Start requires durable launch binding (RESUME-1, SEE-3): `previewed → started` is legal only after the launch-binding append succeeds. Operands/product IDs: `run.previewed`, `run.started`, `runId`, `planRef`, `policyRef`, `trackRef`. Authority: Bootstrap + Records. Back-citation: core/bootstrap.md candidates; core/orchestration.md `previewed → started`.                                                                                                                                         | SRC-002, SRC-006       |
| INV-010  | Invariant     | Launch binding stays immutable across resume (GUARD-1): `stopped → resumed` may continue a run but may not widen or swap launch-time policy, work-profile, or repo-floor bindings. Operands/product IDs: `policyRef`, `workProfileRef`, repo floor. Authority: Bootstrap binding gate + Runner resume gate. Back-citation: bootstrap and orchestration candidates.                                                                                                                                          | SRC-002, SRC-006       |
| INV-011  | Invariant     | Resume requires owner approval after safety-relevant change (RESUME-5, GUARD-2): `stopped → resumed` is illegal after changed rule-governing inputs until approval and fresh evidence are recorded. Operands/product IDs: policy, verification, integration-safety inputs. Authority: Runner resume gate. Back-citation: bootstrap candidates; orchestration `stopped → resumed`.                                                                                                                           | SRC-002, SRC-006       |
| INV-012  | Invariant     | Resume has no double effect (RESUME-3, SEE-3): a resumed run recognizes irreversible actions already recorded and does not perform them again. Operands/product IDs: `stopped → resumed`, irreversible-action records, append-only log. Authority: Records consumed by Runner. Back-citation: orchestration resume edge and run-lifecycle candidates.                                                                                                                                                       | SRC-002, SRC-006       |
| INV-013  | Invariant     | Unattended park becomes a recorded run stop (DOOR-2, LIVE-2, RESUME-4): `parked` work that needs a human must drive the run to `stopped` at a safe checkpoint. Operands/product IDs: work item `parked`, run `started/resumed → stopped`, checkpoint record. Authority: Runner. Back-citation: orchestration stop edges and candidates.                                                                                                                                                                     | SRC-002, SRC-006       |
| INV-014  | Invariant     | The work-item transition set is closed and guard-complete (ISO-1): every legal work-item transition appears with a named guard, and any omitted edge is illegal. Operands/product IDs: `eligible`, `started`, `parked`, `done`, `landed`, `rejected`, `blocked`. Authority: Runner work-item FSM. Back-citation: orchestration work-item table and candidates.                                                                                                                                              | SRC-002, SRC-004       |
| INV-015  | Invariant     | Fence denial fails closed through the lifecycle edge (FENCE-1/2/3): a Fence `deny` decision forces `started → blocked`; denied work never proceeds or widens authority. Operands/product IDs: `deny`, `started → blocked`, bound policy. Authority: Fence decision consumed by Runner. Back-citation: orchestration `started → blocked` and candidates.                                                                                                                                                     | SRC-002, SRC-004       |
| INV-016  | Invariant     | Routed escalation is durable and narrow (DOOR-1/2/3): a routed request moves `started → parked`, survives interruption, and resumes only from a scoped owner grant. Operands/product IDs: `route`, `started → parked`, parked record, approval scope. Authority: Doorbell path + Records. Back-citation: orchestration `started → parked` and candidates.                                                                                                                                                   | SRC-002, SRC-006       |
| INV-017  | Invariant     | Capability proof gates autonomy (EARN-1/2): auto-grantable progress requires fresh positive capability proof for the relevant driver and run context; stale proof reduces autonomy. Operands/product IDs: proof record, driver/run context, grantable path from `started`. Authority: Fence. Back-citation: orchestration candidates and guard notes.                                                                                                                                                       | SRC-002                |
| INV-018  | Invariant     | Blocked work is a durable operator-visible outcome (MERGE-5, SEE-1/2): `blocked` is recorded and surfaced where the owner already works. Operands/product IDs: `blocked`, block reason/evidence, notice or PR-compatible surface. Authority: Runner + Records. Back-citation: orchestration `started → blocked` and candidates.                                                                                                                                                                             | SRC-002, SRC-006       |
| SURF-001 | Surface       | `OperatorControlPort` — the operator entry point: `previewRun`, `startRun(dryRun)`, `inspectRun`. Invariant: one command → one control-plane call → SDK/operator-owned records. Edge holds no run logic.                                                                                                                                                                                                                                                                                                    | SRC-003; RL-8          |
...
| DEL-005  | Delivery      | Operator entry point (CLI + `OperatorControlPort`) for validate / preview / dry-run. Preserves SURF-001, the one-command-one-call-SDK-records invariant.                                                                                                                                                                                                                                                                                                                                                    | SRC-003; RL-8          |
