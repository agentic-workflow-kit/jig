...
| Storage (`PORT-LEDGER`/`PORT-ARTIFACT`) | Conditional appends are decided by expected position; an unknown commit acknowledgement is resolved by reading the position.   | Digest-verified reads answer whether a record or artifact exists; repair or migration only as new authorized Operations.            | A storage reconnection replays no writes; the append condition and content digests make duplicate application detectable.        |
...
flowchart LR
    ...
    subgraph Outside["Configured external mechanism"]
        Mechanism["X-AGENT · X-WORKSPACE · X-VERIFY<br/>X-DELIVERY · X-STORE<br/>MC-conformant provider<br/>[External mechanism]"]
    end
...
path; color is redundant with the stable IDs and bracketed types. `MC` means mechanism-contract
clause and `CB` capability binding.
