...
const fixture = JSON.parse(readFileSync(resolve(rootDir, 'tests/fixtures/gf-003/topology.json'), 'utf8'));
const fakeFixture = JSON.parse(readFileSync(resolve(rootDir, 'tests/fixtures/gf-003/fake-script.json'), 'utf8'));
...
test('GF-003: fixed topology has exactly six units and eleven semantic ports', () => {
...
test('GF-003: source is builder-owned and cannot cross to the controller', () => {
...
test('GF-003: unknown and forbidden authority crossings fail closed', () => {
...
test('GF-003: every fixed crossing passes and every field mutation fails closed', () => {
...
test('GF-003: topology crossing accepts only primitive canonical encodings', () => {
...
test('GF-003: scripted fakes accept only canonical GF-002 frames', () => {
  assert.equal(fakeFixture.fixtureVersion, 'gf-003-fakes.v1');
