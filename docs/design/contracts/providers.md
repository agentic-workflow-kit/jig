...
status: draft — all four seams specified
---
...
The product-posture disclaimer (no public provider API, package, registry, manifest schema, or
semver stability promise today) is stated once, design-side, in
[`README.md`](./README.md#notes); it is not restated here.
...
This file holds the seam contract, organized by seam. How each seam was realized over time — from
jig-internal reference adapters (ADR 0021) through real drivers for Agent and Execution host
(ADR 0022), Forge (ADR 0023), and Work source (ADR 0024) — is recorded in
[`provider-realization-roadmap.md`](./provider-realization-roadmap.md), organized by phase. That
roadmap changes no port shape, owns/implements/must-not rule, or invariant named above; it only
records which adapter realized a seam and when.
...
- `CTX-005` — the four driver seams are treated as ports. CTX-005 records the M5b baseline (only
  the Agent seam exercised, by the scripted stub, with the others as named extension points);
  [`provider-realization-roadmap.md`](./provider-realization-roadmap.md) records that all four
  seams are since specified with reference adapters (ADR 0021), without changing the port shapes
  CTX-005 names.
