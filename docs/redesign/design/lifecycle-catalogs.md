...
stateDiagram-v2
    ...
    Retiring --> Closed: EV-OWNER-DECISION, exact Residual Obligation handoff accepted
...
requires every in-flight target-changing Operation of the holding Story to be attested, confirmed,
cancelled with proof of no effect, or parked as `Uncertain` under `BND-RECOVERY`. Until then the
Story transition still records its destination state, but the authority is retained-but-fenced; the
existing reconciliation-completing Transition releases it durably. This condition is vacuously
satisfied only where the edge structurally cannot have an in-flight target-changing Operation.
...
receipt/failure fact, obligation resolution, and owner-authorized evidence disposal. It cannot
...
exact evidence subject, and validated result digest. Repeated put/get/dispose attestations for that tuple
...
| `EV-ARTIFACT-FACT`              | Artifact facts            | `X-STORE` via `PORT-ARTIFACT` through `CP-MEDIATOR`                    | Evidence subject, artifact Operation | `OPC-ART-PUT`/`OPC-ART-GET` result, certainty, or failure; or `OPC-ART-DISPOSE` digest-verified disposal result, certainty, or failure after owner decision, preservation, retention, obligation, and deployment-wide foreign-reference guards pass. The reference guard proves no other non-disposed Run's recorded evidence manifest or export manifest names the digest; a live foreign reference fails disposal closed and names the owning Runs in the failure reason. Exact subject binding, artifact/content digest, Operation fence, and producer capability are required. A put is adopted into a manifest only after this event commits. |
...
| `EV-EFFECT-CERTAINTY`           | Delivery facts            | `X-DELIVERY` via `PORT-DELIVERY`                                       | Operation                            | Operation identity, fence, certainty classification. For a retained-but-fenced authority in `Suspended`, a fact that establishes the last target-changing Operation as attested, confirmed, cancelled with proof of no effect, or parked `Uncertain` under `BND-RECOVERY` triggers the phase-preserving `Suspended` → `Suspended` Transition that records the durable authority release; it dispatches no Operation.                                                                                                                                                                                                                               |
...
| `EV-OWNER-DECISION`             | Human answers             | `P-OWNER` via `PORT-DECIDE`                                            | Parked request, evidence artifact    | Responder identity/current in-scope grant, exact request binding where applicable, and reason. Disposal authorization is owner-only and binds one exact evidence-artifact digest; it cannot authorize another artifact or widen a provider posture. Story rejection requires a Jig-originated Story-bound request that offered `reject-story`; provider answers cannot reject a Story or authorize a Jig Operation.                                                                                                                                                                                                                                |
...
| `EV-RUN-SUSPEND-DECISION`       | Operator decisions        | `P-OWNER` via `PORT-DECIDE`                                            | Run                                  | Exact Run, `Active` or `Parked` position, responder identity/current grant, and durable reason; the Transition fences dispatch immediately. It releases finalization authority only after every in-flight target-changing Operation of the holding Story is attested, confirmed, cancelled with proof of no effect, or parked as `Uncertain` under `BND-RECOVERY`; otherwise authority remains retained-but-fenced while the Run suspends, awaiting the existing reconciliation-completing facts that can record its phase-preserving durable release.                                                                                             |
...
| `EV-OBLIGATION-RESOLVED`        | Operator decisions        | `P-OWNER` via `PORT-DECIDE`                                            | Residual Obligation                  | Live `ID-OBLIGATION`, responder identity/current grant, completion criteria, and digest-verified resolution-evidence reference. Resolution retains the obligation identity and records its position; after Run settlement it is a post-terminal administrative Transition that cannot alter Run phase, Story state, business-final cut, or outcome.                                                                                                                                                                                                                                                                                                |
...
| `EV-WAKE-AUTHORITY`             | Wake triggers             | `RT-CONTROLLER` durable wake                                           | Story, target                        | Derived from recorded authority release plus `C-ORDER`, or from exact re-approval while the same Story's refreshed finalization authority remains valid.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
| `EV-WAKE-SETTLEMENT`            | Wake triggers             | `RT-CONTROLLER` durable wake                                           | Run, terminal/retiring Story         | Story-scoped derivation begins Retirement from one committed business-terminal root and re-evaluates on every duty-completing durable fact — preservation, session close, authority release, or obligation acceptance — so `Retiring` closes when the last duty completes regardless of order; `EV-WORKSPACE-PRESERVED` remains a valid trigger when it is last. Run-scoped derivation enters `Settling` when all Stories are terminal/retiring and, for `Settling` and `Stopped`, commits the business-final terminal-settlement position once every duty is complete or handed off.                                                              |
| `EV-RECOVERY-OBSERVATION`       | Recovery observations     | `CP-RECOVERY` via the mechanism ports                                  | Operation, Run                       | Reconciliation provenance, fence, certainty classification. For a retained-but-fenced authority in `Suspended`, the fact may complete the last target-changing Operation's attested, confirmed, cancelled-with-proof, or parked-`Uncertain` disposition and trigger the phase-preserving `Suspended` → `Suspended` Transition that records the durable authority release; it dispatches no Operation.                                                                                                                                                                                                                                              |
...
| `OPC-ART-DISPOSE`        | `PORT-ARTIFACT`  | Dispose one digest-identified evidence artifact                   | Irreversible destructive effect                 | Certainty reconciliation. Dispatch requires an exact owner `EV-OWNER-DECISION`, completed preservation and retention eligibility, no open obligation or audit hold, and a verified deployment-wide reference guard proving no other non-disposed Run's recorded evidence manifest or export manifest references the digest. A live foreign reference fails disposal closed and names the owning Runs in the failure reason; after settlement it is a post-terminal administrative Transition. |

### Existing authorizing-transition mapping

The following existing V9/V3 transitions record the intents for catalog rows that are otherwise
phase-preserving. Each intent is inside that Transition's `SCH-TRANSITION`; the later port fact is
validated and committed as the next trigger, never treated as ambient dispatch.

| Operation             | Existing authorizing Transition and resulting fact                                                                                                                                                                                                                                                                             |
| --------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `OPC-SESSION-COLLECT` | The active `Implementing` turn authorized by V9 `Preparing` → `Implementing` records the bounded collection intent in its existing phase-preserving `SCH-TRANSITION`; the collected fact is `EV-SESSION-RESULT` or `EV-SESSION-VERDICT`, which can then commit V9 `Implementing` → `Reviewing` or its existing review outcome. |
| `OPC-SESSION-CLOSE`   | The existing V9 `Landed`/`Blocked`/`Rejected` → `Retiring` `EV-WAKE-SETTLEMENT` Transition records terminal-duty close intent; its completion is the `EV-SESSION-FACT` close result used by the existing settlement derivation.                                                                                                |
| `OPC-WS-PROVISION`    | The existing V9 `Eligible` → `Preparing` `EV-WAKE-CAPACITY` admission Transition records the preparation provision intent; `EV-WORKSPACE-FACT` then supplies the existing `Preparing` → `Implementing` readiness fact.                                                                                                         |
| `OPC-WS-OBSERVE`      | The existing V9 preparation/refresh Transitions while the Story remains `Preparing` or `Refreshing` record the observation intent; their `EV-WORKSPACE-FACT` result supplies the existing `Preparing` → `Implementing` or `Refreshing` → `Reviewing` edge.                                                                     |
| `OPC-WS-PRESERVE`     | The existing V9 `Landed`/`Blocked`/`Rejected` → `Retiring` `EV-WAKE-SETTLEMENT` Transition records preservation intent; `EV-WORKSPACE-PRESERVED` is the existing duty-completing fact.                                                                                                                                         |
| `OPC-WS-RETIRE`       | The existing `Retiring` `EV-WAKE-SETTLEMENT` duty Transition records retire-or-handoff intent only after preservation; its resulting `EV-WORKSPACE-FACT` is re-evaluated by the existing settlement derivation.                                                                                                                |
