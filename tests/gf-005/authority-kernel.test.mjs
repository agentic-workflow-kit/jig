...
const oracle = JSON.parse(
  readFileSync(resolve(import.meta.dirname, '../fixtures/gf-005/authority-oracle.json'), 'utf8'),
);
...
test('GF-005 catalog is literal, immutable, and exhaustive', () => {
...
test('GF-005 reducer is total, deterministic, immutable, and fail closed', () => {
...
test('GF-005 fixed-field bindings accept semantically identical insertion-order variants', () => {
...
test('GF-005 rejects a descriptor-valid event whose ordinary type read throws', () => {
...
test('GF-005 exported validators reject throwing hostile Proxies as FC-INPUT without escape', () => {
...
test('GF-005 rejects descriptor-valid ordinary-read traps at every authority boundary', () => {
...
test('GF-005 preserves a nested FC-FENCE transition failure', () => {
  const stale = {
    ...state,
...
test('GF-005 replay is total and fail-closed at its external step-array boundary', () => {
...
test('GF-005 transition rejects a hostile operations array without escape', () => {
...
test('GF-005 accepts every explicit legal edge and rejects its event complement', () => {
...
test('GF-005 Run catalog validates all V3a phases and realizes every controller-owned Run edge', () => {
...
test('GF-005 has no forgeable promotion or dispatch capability', () => {
...
test('GF-005 replay is byte-stable and rejects descending, duplicate, and gapped committed positions', () => {
