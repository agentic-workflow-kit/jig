...
settlement, only the cataloged Story `Retiring` self-loop consumes `EV-WAKE-SETTLEMENT`: it commits
holder retirement and a post-commit `release-pin` intent without changing Story state. After
terminal settlement, the post-terminal administrative holder-retirement row owns the same wake.
No other pre-terminal phase consumes it.
