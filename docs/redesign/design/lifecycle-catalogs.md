...
| `EV-ARTIFACT-FACT`              | Artifact facts            | `X-STORE` via `PORT-ARTIFACT` through `CP-MEDIATOR` | Evidence subject, artifact Operation | `OPC-ART-PUT`/`OPC-ART-GET` result, certainty, or failure; exact subject binding, artifact/content digest, Operation fence, and producer capability. A put is adopted into a manifest only after this event commits.                                                                                                           |
...
| `EV-OBLIGATION-RESOLVED`        | Operator decisions        | `P-OWNER` via `PORT-DECIDE`                         | Residual Obligation                  | Live `ID-OBLIGATION`, responder identity/current grant, completion criteria, and digest-verified resolution-evidence reference. Resolution retains the obligation identity and records its position; after Run settlement it is a post-terminal administrative Transition that cannot alter the business-final cut or outcome. |
...
separate terminal selector is `FC-TRUST`, whose fixed result is the named trust-root stop.
