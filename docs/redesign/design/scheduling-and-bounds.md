...
| `RC-SESSION`     | Retained role sessions        | Each logical `ID-SESSION`, including every fresh rework assignment; provider-process reuse does not merge units.                                       | Agent mechanism configuration.              | Any value at or above the progress reserve.         |
| `RC-IMPL-TURN`   | Active implementer turns      | Each acknowledged logical implementation or rework assignment on `PORT-SESSION`; same-assignment reconnect/replacement does not consume a second unit. | Agent mechanism configuration.              | Any value at or above the progress reserve.         |
...
| -------------------- | ----------------------------------------------------------------------------------------------------------------------------------- | --------------------------------- | ---------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------ |
| `BND-REWORK`         | Review and rework loops per Story; each transition into `Reworking` allocates the next ordinal exactly once.                        | 2 loops; range 1–5.               | No automatic renewal. Capacity retries, session-open retries, and same-assignment replacement retain the committed ordinal.  | Block the Story; dependents derive `Not run — dependency blocked`.                                                                   |
...
| `BND-RETIRE`         | Retirement or terminal-stop Settlement-overlay attempts per exact duty.                                                             | 3 attempts; range 1–5.            | No renewal; exhaustion creates one `open` Residual Obligation without changing preserved Story state.                        | Mint one `open` Residual Obligation assigned to the accountable owner; never accept handoff or satisfy the duty automatically (I19). |
...
Story `Pending` (dependency eligibility awaiting every prerequisite's confirmed `Landed`) is
**derived structural ineligibility, not a wait**, and is not a seventeenth surface here: it names no
`BND-*` class, no accountable-owner-held bound, and no exhaustion timer. It resolves by derived
eligibility once every prerequisite reaches `Landed`, or, on a prerequisite reaching `Blocked` or
`Rejected`, by the terminal `Not run — dependency blocked` disposition (I14, `CF-BLOCKERS`). It is
observable through the dependency/blocker projection (`OBS-STORY-BOARD`), not `OBS-WAITS`, which
covers only live waits. This finite sixteen-surface inventory enumerates waits; it does not need to
enumerate derived ineligibility, which this clause disambiguates rather than extends.

| #   | Surface                                  | Accountable owner and durable reason/start                                                                                                                                                                                                                                                        | Wake or completion                                                                                                        | Bound/reset and exhaustion                                                                                                                                                                                                                                                                                                                                                                        |
| --- | ---------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | Review/rework loop                       | `CP-TRANSITION`; a `changes-required` verdict or failed required check commits one fresh rework ordinal and deterministic logical-assignment basis.                                                                                                                                               | `CP-SCHEDULER` re-admits through `EV-WAKE-CAPACITY`; open/bind/assignment facts advance through the cataloged self-loops. | `BND-REWORK`, `RC-SESSION`, and `RC-IMPL-TURN`; retries and same-assignment replacement retain the ordinal, a new rework turn does not. Exhaustion blocks the Story and derives dependent non-run outcomes.                                                                                                                                                                                       |
...
| 3   | Target refresh                           | `CP-FINALIZER`; target-basis advance while authority is held records the refresh turn.                                                                                                                                                                                                            | Aligned `SCH-CANDIDATE` fact followed by review/rebind.                                                                   | `BND-REFRESH`, no renewal; enter `RefreshPark`, escalate, and release no dependents.                                                                                                                                                                                                                                                                                                              |
...
| 12  | Retirement or terminal-stop overlay duty | Settlement through `CP-TRANSITION`; terminal duty and accountable owner are recorded in Story Retirement or the one Run-scoped `SCH-SETTLEMENT`.                                                                                                                                                  | `EV-WAKE-SETTLEMENT` or the exact duty-completing fact.                                                                   | `BND-RETIRE`, no renewal; exhaustion mints one owner-accountable `ID-OBLIGATION` in `open`. In terminal `Stopped` it preserves the Story snapshot and business-dispatch fence. Creation is not handoff, does not satisfy the duty, and cannot substitute for Arye's exact acceptance event.                                                                                                       |
...
| 15  | Finalization-authority queue waiter      | `CP-FINALIZER`; `SCH-REGISTRY-RECORD` stores `SCH-CANDIDATE` binding values, eligibility basis, `C-ORDER`, and continuous-starvation start.                                                                                                                                                       | `EV-WAKE-AUTHORITY` after release and comparator selection.                                                               | `BND-WAIT-CAPACITY`; re-evaluation does not reset the clock; exhaustion parks/escalates without target mutation or dependent release.                                                                                                                                                                                                                                                             |
...
Qualifying progress is exactly one durable subject-bound committed fact: a new atomically committed
`SCH-CANDIDATE` through `EV-SESSION-RESULT` or refresh `EV-WORKSPACE-FACT`, adopted evidence
through `EV-ARTIFACT-FACT`, a completed check through `EV-CHECK-OBSERVATION`, or a checkpoint explicitly enumerated in the frozen, digest-bound
...
the open obligation, original bound, owner-only handoff, and current `Settling`, terminal
`Stopped` overlay, or `Retiring` position; it cannot auto-handoff, resolve, close, settle, authorize a
...
existing `EV-WAKE-SETTLEMENT` during the cataloged Story `Retiring` self-loop, the terminal
`Stopped` Settlement-overlay self-loop, or after terminal settlement. That wake commits the holder
retirement and its `release-pin` intent; dispatch follows the commit. No other position consumes
this holder wake. It adds no bound, wait class, or ambient timer surface.
