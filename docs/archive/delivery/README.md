...
# Jig delivery planning archive

This directory preserves completed and superseded delivery tracks. These records explain how prior
implementation phases were sequenced and accepted; they are not the active operating plan.

Archived delivery records:

- [M5b local MVP roadmap, r2](./m5b-local-mvp-r2/) — delivered-state record, org-M5 exit-criteria
  map, and the remaining Phase 4/5 ladder as written at that time. Its provider tail (Phases 6-9)
  moved into the M7 track below.
...
These archived delivery roadmaps derived from:
