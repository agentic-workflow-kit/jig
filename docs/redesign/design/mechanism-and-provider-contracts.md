...
| `CB-STORE`              | `PORT-LEDGER`/`PORT-ARTIFACT` | For Run-scoped `PORT-ARTIFACT`: one named artifact write/read, minted per Operation and validated by `CP-MEDIATOR`. Existing holder class selects and the approved artifact-provider manifest/resource scope binds either the protected configuration/intake or disposable evidence routing context; this adds no persisted identity or schema operand, and a changed, missing, or unverifiable binding fails closed. The pre-Run configuration-artifact exception therefore binds the protected context, exact artifact subject/digest, envelope composition digest, an exchange-attempt key with deterministic start/result variant keys, ordinal, start/deadline/result, predecessor attempt, and `BND-WAIT-MECHANISM`/`BND-RETRY`; the storage mechanism uses `LG-PREFLIGHT-ATTEMPT` to conditional-create/read back immutable `SCH-INTAKE-ACK` variant bytes, `CP-MEDIATOR` validates them, and no Operation, event, Run, intake authority, Transition, or dispatch exists. Capability-proof attempts use the same primitive and protected context inside the provider's own configured exchange without acquiring a reusable store binding. For `PORT-LEDGER`: one intake composition-digest key minted by `CP-INTAKE`; one Run's positions; one realization-bound registry line; or the corresponding witness heads under their declared durability protocols. Those heads include exactly one artifact-reference-index witness line keyed by the approved artifact provider's configured deployment-wide disposable evidence resource scope; pin add/release advances it only as a subordinate clause of the enclosing holder action. |
...
| ------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Operation identity echo (`MC-ECHO`)               | The qualified Transition identity (position plus proposing generation plus record digest), or the registry line / witness head key.                                                                                                                                          | For intake authority, the composition-digest key plus immutable `SCH-INTAKE-ACK` `terminal-ack` digest returned by conditional-create/read. Any mismatched key/digest is rejected.                                                                                     |
...
| Artifact (`PORT-ARTIFACT`)   | Run-scoped immutable put/get/dispose is idempotent or effect-free under exact Operation identity. The existing holder class selects a protected configuration/intake or disposable evidence routing context bound by the approved artifact-provider manifest/resource scope. The five pre-Run classes use the protected context, which rejects disposal, move, and cross-context aliasing. A pre-Run configuration read binds that context, exact subject, expected digest, composition digest, deterministic exchange-attempt and derived variant keys, ordinal, and bounds without creating an Operation/event; `LG-PREFLIGHT-ATTEMPT` conditional-creates or reads back its immutable start/result bytes. In the disposable context, every holder adoption registers its durable pin first: a pin add/release durably flushes and returns its exact lookup `(position, head digest)` to `CP-EVIDENCE`; the artifact mechanism never advances `RT-WITNESS` itself. | Run results return through `EV-ARTIFACT-FACT`; the pre-Run read returns only validated intake evidence. Same-variant-key bytes must be equivalent; mismatch, deadline, predecessor, or integrity failure fails closed. Disposal accepts only the disposable context and uses its authoritative deployment-wide reverse-reference/pin lookup over the six exhaustive disposable holder classes in [evidence handling](./evidence-handling.md), atomically linearized against new pins; a later scan is not a substitute. `CP-EVIDENCE` supplies the returned head to `CP-TRANSITION`, whose commit protocol advances it through `PORT-LEDGER`; only durable matching completion permits the existing holder acknowledgement. Every attempt uses `BND-WAIT-MECHANISM`; retries consume `BND-RETRY`; effectful uncertainty uses `BND-RECOVERY`. | Reconnection or restore never replays an effect blindly. Before any disposable reference adoption, release, or `OPC-ART-DISPOSE`, it verifies the pin lookup chain/head against its independent witness; behind, forked, digest-mismatched, unverifiable, or missing acknowledged content is `FC-TRUST` and a deliberate stop pending exact-head externally governed reconciliation. Without the witness, autonomous restore cannot establish disposable pin currency. Put/dispose reconcile first; a config-read crash re-reads the same protected-context variant key and exhaustion rejects intake before Run creation. Any protected-context address or live disposable pin rejects disposal, while the exact owner authorization for that same disposable address is the sole non-blocking self-reference. Preservation, retention, obligation, and cross-Run reference guards remain mandatory. |
...
`PORT-ARTIFACT` returns the durably flushed pin-mutation proof to `CP-EVIDENCE`, which supplies the
exact head to `CP-TRANSITION`; that existing commit protocol advances it through the witness-line
`PORT-LEDGER` path before the holder may be acknowledged or adopted. The artifact mechanism does
not bypass `PORT-LEDGER` or write `RT-WITNESS` directly, and the witness step grants no authority.
