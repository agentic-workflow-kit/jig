...
| --------------- | --------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------ |
| `CP-INTAKE`     | Intake and preflight              | Conditionally creates or looks up `LG-INTAKE` by composition digest, returns the durable `SCH-INTAKE-ACK`/same `ID-RUN`, validates the complete envelope, runs preflight, and freezes or rejects durably.                                                             | `RUN-RECEIVED`, `RUN-PREFLIGHT`, `RUN-REJECTED` (V3); frozen-envelope and intake-idempotency bindings. | `PORT-INTAKE` inbound; `PORT-LEDGER` only for `LG-INTAKE`.                                                                                       |
| `CP-TRANSITION` | Transition engine                 | Executes the single-transition cycle for every accepted trigger: validate lifecycle/grant, classify rule surfaces, decide, record, then adopt and dispatch. It alone authorizes review-publication intents from `Reviewing`/`Blocked` without finalization authority. | `FLOW-VALIDATE`, `FLOW-DECIDE`, `FLOW-RECORD`, `FLOW-ADOPT` (V3c); I4, I5.                             | `PORT-LEDGER` (sole writer; conditional ordered appends).                                                                                        |
...
| `CP-FINALIZER`  | Finalization authority manager    | Manages the single target-scoped finalization authority: grant in deterministic order, fence binding, bounded refresh, rebinding, and release. It proposes only post-acceptance `OPC-DEL-*` intents and is never involved in review publication.                      | `C-FINALIZER` (V4); I12.                                                                               | None directly; authorized target effects dispatch through `CP-MEDIATOR`.                                                                         |
