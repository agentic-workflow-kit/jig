...
| `Active`, `Parked`, `Suspended`, `Settling`, or pre-terminal `Stopped` → same phase | `EV-BOUND-EXHAUSTED`                                                                                                                                                                                                                                                                                                  | An exact live `open` Residual Obligation reached its original `BND-WAIT-DECISION` deadline and no prior exhaustion fact exists                                                                                                                    | Record one durable overdue re-escalation and notice basis for the same `ID-OBLIGATION`, original start/deadline, accountable owner, and owner-only handoff authority; do not reset the wait, change obligation status, or complete settlement.                      |
...
overdue-obligation re-escalation, obligation handoff acceptance or resolution, and owner-authorized
evidence disposal. They cannot alter any Run
...
| Evidence-disposal authorization           | `EV-OWNER-DECISION`      | Arye alone authorizes one exact disposable-context digest after holder-class routing and provider-resource binding, preservation, retention, obligation, audit-hold, the six-class deployment-wide live-pin guard, and current witnessed pin-head guard pass; protected-context subjects are ineligible; append only the disposal intent. |
...
| `Completed`                 | `Settling` at the terminal-settlement position                      | Only the settlement-authorized audit-export put and receipt/failure, overdue-obligation re-escalation, obligation handoff acceptance or resolution, and owner-authorized evidence disposal. None may change phase, Story state, business-final cut, outcome, authority, or dependency. |
