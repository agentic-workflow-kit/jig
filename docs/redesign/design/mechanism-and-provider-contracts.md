...
| `MC-RECONNECT`  | A session mechanism resumes by identity or attests loss. A replacement binds the same `ID-PRINCIPAL` and assignment with recorded provenance. Pending `ID-PARK` requests survive and answers follow that identity; if context cannot be restored, Jig records cancel-and-reissue with predecessor/successor lineage. Reconnection never launders contribution history or silently drops a request.                                                                                   |
...
| ----------------------- | ----------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `CB-SESSION`            | `PORT-SESSION`                | One role assignment, its bounded inputs, one session on one configured Agent provider, its exact native permission-posture reference, and only the authority allowed by its approved manifest. A response binding pins `ID-PARK`, originating `ID-PRINCIPAL`/assignment, current same-principal session, exact answer, and any replacement or cancel-and-reissue lineage.                                                                                                                                                                                                   |
