...
- **Intake is conditional-create by composition digest.** The first submission durably creates one
  ...
  `ID-RUN` is derived from that immutable mapping's zero-padded `LG-INTAKE` create position and
  the first 16 lowercase-hex characters of the composition digest, retaining the `run-<token>`
  shape. No duplicate intake event or second Run may be created for the same digest.
...
| `SCH-INTAKE-ACK`         | Durable intake acknowledgement           | Envelope composition digest as the sole submission identity, its proposal digest and bound owner-approval record, `ID-RUN`, accepted/rejected preflight disposition and reason, acknowledgement digest, and conditional-create or digest-lookup proof. The acknowledgement digest is the hash of canonical acknowledgement bytes with its acknowledgement-digest field excluded; same-digest readback compares that exact digest domain and is byte-equivalent. An accepted acknowledgement also embeds the canonical frozen `SCH-ENVELOPE` bytes, including its explicit `successorLineage` or `absent` genesis value, and complete Run-ledger genesis/rebuild basis; recovery needs no mutable or separately authoritative input.                                                                                                                                                                                                                                                                                                                                                                       |
...
| `SCH-CAPABILITY-PROOF`   | Compose-time capability proof            | Exact provider identity and build digest, approved `ID-MANIFEST`, environment fingerprint, requested capability, policy minimum, result, proof-basis/evidence reference, and production position/age basis. It is immutable recorded evidence referenced from the envelope; missing, stale, negative, or subject-mismatched proof fails intake preflight closed.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
| `SCH-DECISION`           | Human/operator decision                  | Exact Run, parked-request, Story, notice, `ID-OBLIGATION`, or exact evidence-artifact digest subject; decision kind and reason; responder identity; for a non-owner, current `ID-GRANT`; exact request binding where applicable; scoped permission/question answer, continuation, reject-story, suspend, resume, terminal stop, acknowledgement, snooze, obligation resolution with evidence, or owner-only disposal authorization naming that artifact digest. An override is a scoped owner or in-grant delegated decision selecting a different offered choice than Jig's derived selection for an exact parked request or Story-bound decision point, recorded through `EV-OWNER-DECISION` under its existing guards; it never bypasses cataloged Transitions and introduces no decision kind. Provider answers grant no Jig Operation and cannot change posture.                                                                                                                                                                                                                                     |
...
| `SCH-INTAKE-ACK`         | `CP-INTAKE` conditionally creates or returns it in `LG-INTAKE`.                                                                          | `RT-OPERATOR`, recovery, derived ledger creation, and controller start. | `CP-INTAKE` preflight plus `PORT-LEDGER` conditional-create/read proof.                                                                                                                                                                                                                       |
...
| `SCH-CONFIG-ARTIFACT`    | Preset or prompt/role-artifact author; a generator produces only its declared generated content.                                         | Envelope Builder, work-profile resolution, and `CP-INTAKE`.             | `CP-INTAKE` requests the pre-Run `CP-MEDIATOR`-validated `PORT-ARTIFACT` digest-verified read, bound to exact subject, expected digest, and composition digest, then records the carrier subject and digest in `SCH-INTAKE-ACK`; envelope validation rejects a missing or mismatched carrier. |
| `SCH-PROVIDER-AUTHORITY` | Provider authority author, with owner approval bound to the manifest.                                                                    | `EP-PROVIDERS`, session binding, and `CP-INTAKE`.                       | `EP-PROVIDERS`, then `CP-INTAKE`.                                                                                                                                                                                                                                                             |
...
| `SCH-CAPABILITY-PROOF`   | Configured provider through its own mechanism-port exchange.                                                                             | `EP-PROVIDERS` and `CP-INTAKE`.                                         | `EP-PROVIDERS` acquisition/revalidation, then `CP-INTAKE` freshness and subject binding.                                                                                                                                                                                                      |
...
| `SCH-EVIDENCE`           | Evidence producer supplies attributed content; `CP-TRANSITION` records the manifest only after the artifact fact commits.                | Acceptance, audit export, retention, and projections.                   | `CP-MEDIATOR` validates artifact facts; `CP-TRANSITION` validates manifest adoption.                                                                                                                                                                                                          |
...
| `SCH-DECISION`           | Owner or currently authorized delegate through `PORT-DECIDE`.                                                                            | `CP-TRANSITION`, escalation resolution, and projections.                | `CP-ESCALATION` validates identity, grant, exact subject, and lifecycle position.                                                                                                                                                                                                             |
| `SCH-OBLIGATION`         | `CP-TRANSITION` from a recorded duty-completing or handoff fact.                                                                         | Retirement, export, publication, and projections.                       | `CP-TRANSITION` validates origin, completion criteria, and status transition.                                                                                                                                                                                                                 |
...
| `SCH-AUDIT-EXPORT`       | `CP-TRANSITION` records the create-once export after artifact facts commit.                                                              | `PORT-PUBLISH`, export retrieval, and audit readers.                    | `CP-TRANSITION` validates terminal cut and manifest; `PORT-ARTIFACT` verifies referenced artifacts.                                                                                                                                                                                           |
