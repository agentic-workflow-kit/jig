...
| `EV-OWNER-DECISION`        | Human answers             | `P-OWNER` via `PORT-DECIDE`           | Parked request              | Responder identity, delegated scope, exact request binding; a provider answer cannot widen the session posture or authorize a Jig Operation.                    |
...
| `OPC-SESSION-RESPOND` | `PORT-SESSION`   | Return one scoped Doorbell answer to its originating provider request | Irreversible session interaction                | Identity lookup by session and parked-request identity; redispatch is idempotent, and the answer cannot change the frozen provider posture.                                                                             |
