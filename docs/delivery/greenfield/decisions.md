...
| DR-5   | Configuration / GF-022, GF-020, GF-025, GF-026, GF-039, GF-047, GF-060, GF-061 | Initial qualified providers are structured-file source, file stores, local Git workspace, local verifier, Codex sessions, and GitHub delivery. | Other provider modes need their own exact provider gate; omission never waives proof.              | Provider remains unconfigurable.                     | Supported profile and GF-062.   |
...
- Structured-file source, file storage, local Git workspace, local verifier, Codex sessions, and
  GitHub delivery as DR-5-qualified providers.
- Terminal/file notice as a DR-12 presentation channel proven by notice/block-surfacing suites,
  not a `CF-GATE-PROVIDER` subject.
...
GF-025 qualifies the selected local file ledger, registry, and independently trusted witness through
`PORT-LEDGER` and `CF-MECH-LEDGER`. GF-026 separately qualifies the selected local file artifact
provider through `PORT-ARTIFACT` and `CF-MECH-ARTIFACT`. GF-023 must consume both exact provider
gates before proposal approval can select a store. Arye's approved initial profile is a plan
selection, never the implementation-time owner of a delegated DR.
