...
| `ID-EVENT`      | Event identity         | `<run>/event/<ledger position>`                           | Minted only when `CP-MEDIATOR` validation accepts the event and `CP-TRANSITION` commits it; the ledger position is both its collision-free ordinal and the sole trigger-ordering key. Before that validation-commit an event has no identity and no effect.                                             |
...
| ------------------------ | ---------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `SCH-PLAN`               | Execution Plan                           | Story set with unique stable Story keys; dependency edges; per-Story done conditions referencing frozen-policy check classes; exactly one track/policy reference; and the plan digest. The semantic contract and validation are design-owned and non-delegable; wire encoding and serialization are explicitly delegated to engineering.                                                             |
