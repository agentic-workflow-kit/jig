...
| Ledger (`PORT-LEDGER`)       | Conditional appends are decided by expected position; an unknown commit acknowledgement is resolved by reading the position. Registry storage attests the canonical realization descriptor used to derive `ID-REGISTRY`, and a mismatch fails preflight. Witness lines advance durably before append acknowledgement and are independently readable.                                                                                                                                                           | Strict readback classifies the exact proposed record/position; registry-first repair and migration use new records, never mutation.                                                                                                                                                                                                                                                                                                                                                                                                                                                      | Reconnection replays no writes; append conditions, staged record digests, and witness heads expose duplicate application and rollback.                                                                                                                                                                                                                                                                                                          |
| Artifact (`PORT-ARTIFACT`)   | Run-scoped immutable put/get/dispose is idempotent or effect-free under exact Operation identity. Separately, the pre-Run configuration read binds exact subject, expected digest, composition digest, deterministic exchange-attempt and derived variant keys, ordinal, and bounds without creating an Operation/event; `LG-PREFLIGHT-ATTEMPT` conditional-creates or reads back its immutable start/result bytes. Every address-bearing adoption registers its durable pin before the holder can be adopted. | Run results return through `EV-ARTIFACT-FACT`; the pre-Run read returns only validated intake evidence. Same-variant-key bytes must be equivalent; mismatch, deadline, predecessor, or integrity failure fails closed. Disposal uses the authoritative deployment-wide reverse-reference/pin lookup over the eleven exhaustive holder classes in [evidence handling](./evidence-handling.md), atomically linearized against new pins; a later scan is not a substitute. Every attempt uses `BND-WAIT-MECHANISM`; retries consume `BND-RETRY`; effectful uncertainty uses `BND-RECOVERY`. | Reconnection never replays an effect blindly. Put/dispose reconcile first; a config-read crash re-reads the same variant key and exhaustion rejects intake before Run creation. Any monotonic pre-Run protection or live holder pin rejects disposal, while the exact owner authorization for that same disposal is the sole non-blocking self-reference. Preservation, retention, obligation, and cross-Run reference guards remain mandatory. |
