...
| Arye or recorded delegate      | Authorize, Decide                           | Architecture approval, policy or authority exceptions, imports, named escalations, stops, reopens, and bounded choices within the exact current `ID-GRANT` scope. | Routine implementation, verification, delivery, expired/revoked/escaped grant scope, or implicit ambient intervention. |
